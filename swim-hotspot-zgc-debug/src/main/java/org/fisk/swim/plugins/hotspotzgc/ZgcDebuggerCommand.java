...
final class ZgcDebuggerCommand implements DebuggerCommandExtension {
  ...
  public String description() {
    return "zgc phases | zgc zpointer <expression> | zgc remap <expression>";
  }

  @Override
  public boolean handles(String command) {
    return command.equals("zgc") || command.startsWith("zgc ");
  }
  ...
  public String execute(String command, DebuggerCommandContext context) throws Exception {
    ...
    if (argument.equals("phases")) {
      return RuntimeState.read(context).renderPhases();
    }
    if (argument.startsWith("zpointer ")) {
      return inspect(context, argument.substring("zpointer ".length()));
    }
    if (argument.startsWith("remap ")) {
      return remap(context, argument.substring("remap ".length()));
    }
    throw new IllegalArgumentException(
        "Usage: zgc phases | zgc zpointer <colored-pointer-expression> | zgc remap"
            + " <colored-pointer-expression>");
  }

  private static String inspect(DebuggerCommandContext context, String expression)
      throws Exception {
    if (expression.isBlank()) {
      throw new IllegalArgumentException("zgc zpointer requires a colored pointer expression");
    }
  ...
  private static String remap(DebuggerCommandContext context, String expression) throws Exception {
    ...
    return state.describe(pointer)
        + locations
        + "\n(Java implementation of ZBarrier::make_load_good)";
  }

  private static long evaluateUnsigned(DebuggerCommandContext context, String expression)
      throws Exception {
  ...
  private static long readWord(DebuggerCommandContext context, long address) throws Exception {
    ...
    return lastHex(
        context.executeBackendCommand("x/gx " + hex(address)),
        "Could not read ZGC memory at " + hex(address));
  }
  ...
  private record RuntimeState(
      long remapped,
      long remappedYoungMask,
      long remappedOldMask,
      long markedYoung,
      long markedOld,
      long finalizable,
      long remembered,
      ...
      long markBad,
      long storeBad,
      long addressOffsetMask,
      long addressHeapBase,
      String youngPhase,
      String oldPhase) {
    static RuntimeState read(DebuggerCommandContext context) throws Exception {
      return new RuntimeState(
          evaluateUnsigned(context, "ZPointerRemapped"),
              evaluateUnsigned(context, "ZPointerRemappedYoungMask"),
          evaluateUnsigned(context, "ZPointerRemappedOldMask"),
              evaluateUnsigned(context, "ZPointerMarkedYoung"),
          evaluateUnsigned(context, "ZPointerMarkedOld"),
              evaluateUnsigned(context, "ZPointerFinalizable"),
          evaluateUnsigned(context, "ZPointerRemembered"),
              evaluateUnsigned(context, "ZPointerLoadBadMask"),
          evaluateUnsigned(context, "ZPointerMarkBadMask"),
              evaluateUnsigned(context, "ZPointerStoreBadMask"),
          evaluateUnsigned(context, "ZAddressOffsetMask"),
              evaluateUnsigned(context, "ZAddressHeapBase"),
    ...
    String describe(long pointer) {
      ...
      return "ZGC colored pointer "
          ...
          + hex(remapBits)
          + " (active="
          + yes((remapBits & remapped) != 0)
          + ")\n"
          + "  marked: young="
          + yes((pointer & markedYoung) != 0)
          + ", old="
          + yes((pointer & markedOld) != 0)
          + ", finalizable="
          + yes((pointer & finalizable) != 0)
          + "\n"
          + "  remembered: "
          + yes((pointer & REMEMBERED_MASK) == remembered)
          + "\n"
          + "  barriers: load="
          + goodness(pointer, loadBad)
          + ", mark="
          + goodness(pointer, markBad)
          + ", store="
          + goodness(pointer, storeBad)
          + "\n"
          + renderPhases();
    }

    RemapResult resolveLoadGood(long pointer, DebuggerCommandContext context) throws Exception {
      if (pointer == 0 || (pointer & loadBad) == 0) {
        return new RemapResult(uncolor(pointer), uncolor(pointer), true);
      }
      ...
      if (forwarding == 0) {
        return new RemapResult(address, null, false);
      }
      ForwardingLookup lookup =
          lookupForwardedOffset(forwarding, address & addressOffsetMask, context);
      return lookup.found()
          ? new RemapResult(address, addressHeapBase | lookup.toOffset(), false)
          : new RemapResult(address, null, false);
    }

    private String remapGeneration(long pointer, long address, DebuggerCommandContext context)
        throws Exception {
      long bits = rawRemapBits(pointer);
      if ((bits & remappedOldMask) != 0) {
        return "young";
      }
      if ((bits & remappedYoungMask) != 0) {
        return "old";
      }
      if ((pointer & REMEMBERED_MASK) == REMEMBERED_MASK) {
        return "old";
      }
    ...
    private long forwardingFor(String generation, long address, DebuggerCommandContext context)
        throws Exception {
      long generationAddress = evaluateUnsigned(context, "(uintptr_t)ZGeneration::_" + generation);
      if (generationAddress == 0) {
        throw new IllegalStateException(
            "ZGC " + generation + " generation is unavailable in this core");
      }
      long map =
          evaluateUnsigned(
              context,
              "(uintptr_t)((ZGeneration*)"
                  + hex(generationAddress)
                  + ")->_forwarding_table._map._map");
      return readWord(
          context, map + ((address & addressOffsetMask) >>> GRANULE_SHIFT) * Long.BYTES);
    }

    private ForwardingLookup lookupForwardedOffset(
        long forwarding, long fromOffset, DebuggerCommandContext context) throws Exception {
      long start =
          evaluateUnsigned(
              context, "(uintptr_t)((ZForwarding*)" + hex(forwarding) + ")->_virtual._start");
      long alignmentShift =
          evaluateUnsigned(
              context,
              "(uintptr_t)((ZForwarding*)" + hex(forwarding) + ")->_object_alignment_shift");
      long entryCount =
          evaluateUnsigned(
              context, "(uintptr_t)((ZForwarding*)" + hex(forwarding) + ")->_entries._length");
      long forwardingSize = evaluateUnsigned(context, "sizeof(ZForwarding)");
      if (entryCount == 0 || (entryCount & (entryCount - 1)) != 0) {
        throw new IllegalStateException("Invalid ZGC forwarding table size: " + entryCount);
      }
      ...
      for (long probes = 0;
          probes < entryCount;
          probes++, cursor = (cursor + 1) & (entryCount - 1)) {
        long entry = readWord(context, entries + cursor * Long.BYTES);
        if ((entry & 1) == 0) {
          return new ForwardingLookup(false, 0);
        }
        if ((entry >>> 46) == fromIndex) {
          return new ForwardingLookup(true, (entry >>> 1) & FORWARDING_TO_OFFSET_MASK);
        }
      }
      return new ForwardingLookup(false, 0);
    }
    ...
    long uncolor(long pointer) {
      ...
      int shift =
          switch (index) {
            ...
            default ->
                throw new IllegalArgumentException(
                    "Invalid ZGC remap-bit pattern " + hex(pointer & REMAPPED_MASK));
          };
    ...
    private static String goodness(long pointer, long badMask) {
      return (pointer & badMask) == 0 ? "good" : "bad";
    }
  }
  ...
  private static String generationPhase(DebuggerCommandContext context, String generation)
      throws Exception {
    long address = evaluateUnsigned(context, "(uintptr_t)ZGeneration::_" + generation);
    long phase =
        evaluateUnsigned(context, "(uintptr_t)((ZGeneration*)" + hex(address) + ")->_phase");
