    ...
    static SwimHelpChapter chapter() {
        return new SwimHelpChapter("clangd-lsp", "C/C++ LSP",
                ...
                List.of(
                        ...
                        section("Refreshing compile commands",
                                "After regenerating compile_commands.json, run :lsp-restart from a C or C++ buffer. This restarts only clangd for the current project, reloads the compilation database, and reopens the active document; it does not reload SWIM or disturb your layout. :lsp-reload is an alias.",
                                ":lsp-restart"),
                        section("Ignoring unsupported compiler arguments",
                                "Remove compiler arguments for clangd only with clangd.remove_compile_arguments in the project .swim file. SWIM writes a private filtered compilation database; it does not change your build or compile_commands.json. Restart clangd after changing the setting.",
                                "clangd.remove_compile_arguments = -fno-lifetime-dse\n:lsp-restart"),
