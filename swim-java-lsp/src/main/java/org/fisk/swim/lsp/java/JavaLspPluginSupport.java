...
public final class JavaLspPluginSupport {
  ...
  private static final JavaLSPClient UNAVAILABLE =
      new JavaLSPClient(
          new JavaLspProvider() {
            @Override
  ...
  public static void preload(SwimPluginPreloadContext context) {
    ...
    context.registerPreloadResource(
        LanguagePluginRegistry.register(
            "java", PLUGIN_ID, JavaLspPluginSupport::createLanguageMode));
    registerJavaKey(
        context, "g d", "go to definition", "lsp-definition", JavaLspPluginSupport::goToDefinition);
    registerJavaKey(
        context, "g r", "find references", "lsp-references", JavaLspPluginSupport::findReferences);
    ...
    registerJavaKey(
        context, "<SPACE> , l", "code lens", "lsp-code-lens", JavaLspPluginSupport::codeLens);
    registerJavaKey(
        context,
        "<SPACE> , f",
        "format document",
    ...
    registerJavaKey(
        context, "<SPACE> , R", "rename symbol", "lsp-rename", JavaLspPluginSupport::rename);
    registerJavaKey(
        context,
        "<SPACE> , n",
        "inlay hints",
    ...
    registerJavaKey(
        ...
        "<SPACE> e a",
        "generate accessors",
        "lsp-generate-accessors",
        JavaLspPluginSupport::generateAccessors);
    registerJavaKey(
        context,
        "<SPACE> e s",
        "generate toString",
        "lsp-generate-tostring",
        JavaLspPluginSupport::generateToString);
    registerJavaKey(
        context, "<SPACE> e l", "code lens", "lsp-code-lens", JavaLspPluginSupport::codeLens);
  }
  ...
  public static LanguageMode createLanguageMode(Path path) {
    ...
    LogFactory.createLog()
        .debug(
            "Creating Java language mode for {} using client {}", path, lsp.getClass().getName());
    if (lsp.isEnabled() && !lsp.hasStarted()) {
      try {
        ...
        LogFactory.createLog()
            .error(
                "Failed to initialize Java LSP for project "
                    + ProjectPaths.getProjectRootPath(path),
                e);
        lsp.disable();
      }
    }
  ...
  public static void makeFinal() {
    withActiveJavaWindow(
        "lsp code action", client -> client.makeFinal(Window.getInstance().getBufferContext()));
  }

  public static void generateAccessors() {
    withActiveJavaWindow(
        "lsp code action",
        client -> client.generateAccessors(Window.getInstance().getBufferContext()));
  }

  public static void generateToString() {
    withActiveJavaWindow(
        "lsp code action",
        client -> client.generateToString(Window.getInstance().getBufferContext()));
  }

  public static void codeLens() {
    withActiveJavaWindow(
        "lsp code lens", client -> client.codeLens(Window.getInstance().getBufferContext()));
  }

  private static void registerJavaKey(
    ...
    context.registerKeyBinding(
        new SwimPluginKeyBinding(
            key, "LSP", summary, commandName, JavaLspPluginSupport::isJavaBuffer, action));
  }
  ...
  private static void withActiveJavaWindow(
      String editorDriveAction, Consumer<JavaLSPClient> action) {
    Window window = Window.getInstance();
