...
public final class SwimCommandRegistry {
    ...
    public static AutoCloseable register(String pluginId, SwimCommand command) {
        if (pluginId == null || pluginId.isBlank() || command == null || command.name() == null
        ...
        if (COMMANDS.putIfAbsent(key, registration) != null) {
            throw new IllegalArgumentException("Duplicate plugin command: " + key);
        }
    ...
    public static List<Registration> list() {
        return COMMANDS.values().stream().sorted(Comparator.comparing(r -> r.command().name())).toList();
    }
    public static void unregisterPlugin(String pluginId) {
        COMMANDS.entrySet().removeIf(e -> pluginId.equals(e.getValue().pluginId()));
    }
}
