...
public final class UiTheme {
  ...
  private static String resolveColorSpec(
      String role, Map<String, String> rawSpecs, Map<String, Boolean> visiting) {
  ...
  private static String canonicalRole(String value) {
    ...
    role = role.toLowerCase(Locale.ROOT).replace('-', '.').replace('_', '.');
  ...
  private static Map<String, Consumer<TextColor>> colorSetters() {
    ...
    setters.put(
        "completion.selection.background", value -> COMPLETION_SELECTION_BACKGROUND = value);
    setters.put(
        "completion.selection.foreground", value -> COMPLETION_SELECTION_FOREGROUND = value);
  ...
  static void appendSegment(
      AttributedString line, String text, TextColor foreground, TextColor background) {
    line.append(" " + text + " ", foreground, background);
  }

  static void appendRightSeparator(
      AttributedString line, TextColor fromBackground, TextColor toBackground) {
    line.append(Powerline.SYMBOL_FILLED_RIGHT_ARROW, fromBackground, toBackground);
  }

  static void appendLeftSeparator(
      AttributedString line, TextColor fromBackground, TextColor toBackground) {
    line.append(Powerline.SYMBOL_FILLED_LEFT_ARROW, fromBackground, toBackground);
  }

  public static void fillRow(
      TerminalGraphics graphics, Point point, int width, TextColor background) {
    ...
    graphics.fillRow(
        ...
        width,
        new AnsiStyle(
            AnsiColour.DEFAULT, AnsiColour.fromTextColor(background), false, false, false));
  }
  ...
  static void drawLine(
      ...
      TextColor paddingForeground,
      TextColor paddingBackground) {
    if (graphics == null || width <= 0) {
      return;
    }
    AttributedString output =
        line.length() > width ? line.slice(0, width) : AttributedString.create(line);
