...
public class LiveSearchPanelView extends View implements KeyBindingHintProvider {
  public record Entry(
      String kind, String label, String detail, TextColor accent, Runnable action) {}
  ...
  private final Cursor _cursor =
      new Cursor(null) {
        ...
        public int getXOnScreen() {
          return cursorScreenPosition().getX();
        }

        @Override
        public int getYOnScreen() {
          return cursorScreenPosition().getY();
        }
    ...
    _responders.addEventResponder(
        new EventResponder() {
          ...
          public Response processEvent(KeyStrokes events) {
            if (events.remaining() != 0 || events.current().getKeyType() != KeyType.Character) {
              return Response.NO;
            }
            _character = events.current().getCharacter();
            return Response.YES;
          }
  ...
  public void setOnQueryChanged(BiConsumer<String, Long> callback) {
    _onQueryChanged = callback;
  }
  ...
  public void draw(Rect rect) {
    ...
    header.append(
        " " + _results.size() + (_searching ? "+ results " : " results "),
        UiTheme.ACCENT_BLUE,
        UiTheme.SURFACE_ACCENT);
    UiTheme.drawLine(
        graphics, rect.getPoint(), width, header, UiTheme.TEXT_MUTED, UiTheme.SURFACE_ACCENT);
    var queryLine = new AttributedString();
    queryLine.append(" query ", UiTheme.TEXT_ON_ACCENT, UiTheme.ACCENT_GOLD);
    queryLine.append(
        _query.isEmpty() ? " " + _emptyHint : " " + _query,
        _query.isEmpty() ? UiTheme.TEXT_MUTED : UiTheme.TEXT_PRIMARY,
    ...
    UiTheme.drawLine(
        ...
        UiTheme.TEXT_MUTED,
        UiTheme.SURFACE_MUTED);
    int height = Math.max(0, rect.getSize().getHeight() - 2);
    clampSelection(height);
    for (int row = 0; row < height; row++) {
      drawRow(graphics, width, row, _start + row);
    }
    if (_query.isEmpty() && height > 0) {
      drawMessage(graphics, width, "  " + _emptyHint);
    } else {
      if (!_query.isEmpty() && !_searching && _results.isEmpty() && height > 0) {
  ...
  private void drawRow(
      org.fisk.swim.terminal.TerminalGraphics graphics, int width, int row, int index) {
    int y = getBounds().getPoint().getY() + 2 + row;
    ...
    Entry entry = _results.get(index);
    ...
    line.append(
        selected ? "▌ " : "  ",
        selected ? UiTheme.PANEL_SELECTION_ACCENT : UiTheme.TEXT_SUBTLE,
        background);
    line.append(
        entry.kind() + " ", selected ? UiTheme.PANEL_SELECTION_ACCENT : entry.accent(), background);
    line.append(entry.label(), primary, background);
    if (entry.detail() != null && !entry.detail().isBlank()) {
      line.append("  " + entry.detail(), muted, background);
    }
    if (line.length() < width) {
      line.append(" ".repeat(width - line.length()), muted, background);
    }
    UiTheme.drawLine(
        graphics,
        Point.create(getBounds().getPoint().getX(), y),
        width,
  ...
  private void drawMessage(
      org.fisk.swim.terminal.TerminalGraphics graphics, int width, String text) {
    UiTheme.drawLine(
        graphics,
        Point.create(getBounds().getPoint().getX(), getBounds().getPoint().getY() + 2),
        width,
        AttributedString.create(text, UiTheme.TEXT_MUTED, UiTheme.SURFACE_BACKGROUND),
        UiTheme.TEXT_MUTED,
  ...
  private void refreshResults() {
    ...
    setNeedsRedraw();
    if (_onQueryChanged != null) {
      _onQueryChanged.accept(_query.toString(), _generation);
    }
  }
  ...
  private void clampSelection(int height) {
    if (_results.isEmpty()) {
      _selection = _start = 0;
      return;
    }
    _selection = Math.max(0, Math.min(_selection, _results.size() - 1));
    if (_selection >= _start + height) {
      _start = _selection - height + 1;
    } else {
      if (_selection < _start) {
        _start = _selection;
      }
    }
    _start = Math.max(0, Math.min(_start, Math.max(0, _results.size() - Math.max(1, height))));
  }

  private void openSelection() {
    if (_selection >= 0
        && _selection < _results.size()
        && _results.get(_selection).action() != null) {
      _results.get(_selection).action().run();
    }
    close();
  }
  ...
  private Point cursorScreenPosition() {
    int x = getBounds().getPoint().getX(), y = getBounds().getPoint().getY();
    ...
    return Point.create(
        x + Math.min(Math.max(0, getBounds().getSize().getWidth() - 1), 8 + _query.length()),
        y + 1);
  }
}
