...
public class ProjectSearchPanelView extends View implements KeyBindingHintProvider {
  ...
  private final java.util.concurrent.atomic.AtomicInteger _acceptedResults =
      new java.util.concurrent.atomic.AtomicInteger();
  private final ConcurrentLinkedQueue<ProjectSearch.Match> _pendingResults =
  ...
  private static ProjectSearchPanelView create(
      Rect bounds, Path startPath, boolean editableResults) {
  ...
  private ProjectSearchPanelView(
      Rect bounds, ProjectSearch projectSearch, boolean editableResults) {
    super(bounds);
    ...
    _responders.addEventResponder(
        "<ENTER>",
        "Results",
        editableResults ? "edit all results" : "open selected",
        this::openSelection);
    _responders.addEventResponder(
        "<BACKSPACE>",
        "Search",
  ...
  boolean awaitSearchCompletionForTests(long timeoutMillis) throws InterruptedException {
    ...
    while (_pendingResults.isEmpty() && _searching && System.nanoTime() < deadline) {
      Thread.sleep(5);
    }
  ...
  public void draw(Rect rect) {
    ...
    header.append(
        " " + _results.size() + (_searching ? "+ matches " : " matches "),
        UiTheme.ACCENT_BLUE,
    ...
    queryLine.append(
        _query.length() == 0 ? " type to search project text" : " " + _query,
    ...
    for (int row = 0; row < listHeight; row++) {
      ...
      UiTheme.drawLine(
          graphics,
          Point.create(rect.getPoint().getX(), y),
          width,
          line,
          UiTheme.TEXT_MUTED,
          background);
    ...
    if (_query.length() == 0 && listHeight > 0) {
      UiTheme.drawLine(
          ...
          AttributedString.create(
              "  search across visible project files",
              UiTheme.TEXT_MUTED,
              UiTheme.SURFACE_BACKGROUND),
          UiTheme.TEXT_MUTED,
          UiTheme.SURFACE_BACKGROUND);
    } else {
      if (_query.length() > 0 && _results.isEmpty() && listHeight > 0) {
        UiTheme.drawLine(
            graphics,
            Point.create(rect.getPoint().getX(), rect.getPoint().getY() + 2),
            width,
            AttributedString.create(
                "  no matches for current query", UiTheme.TEXT_MUTED, UiTheme.SURFACE_BACKGROUND),
            UiTheme.TEXT_MUTED,
            UiTheme.SURFACE_BACKGROUND);
      }
    }
  ...
  AttributedString buildResultRowForTest(int width, int index, boolean selected) {
    TextColor background =
        selected ? UiTheme.PANEL_SELECTION_BACKGROUND : UiTheme.SURFACE_BACKGROUND;
  ...
  private AttributedString resultRow(int width, int index, boolean selected, TextColor background) {
    ...
    line.append(
        ...
        background);
    line.append(
        " " + leftPad(Integer.toString(match.lineNumber()), lineNumberWidth()) + " ",
        lineColor,
        background);
  ...
  private void refreshResults() {
    ...
    Thread.ofVirtual()
        .start(
            () -> {
              _projectSearch.search(
                  query,
                  matches -> publishMatches(generation, matches),
                  () -> _closed.get() || generation != _searchGeneration.get());
              EventThread.getInstance()
                  .enqueue(
                      new RunnableEvent(
                          () -> {
                            if (generation != _searchGeneration.get()) {
                              return;
                            }
                            _searching = false;
                            updateQuickfix();
  ...
  private boolean reserveResultSlot() {
    for (int current; (current = _acceptedResults.get()) < MAX_RESULTS; ) {
      if (_acceptedResults.compareAndSet(current, current + 1)) {
        return true;
      }
    }
    return false;
  ...
  private void schedulePendingFlush(long generation) {
    if (!_resultFlushScheduled.compareAndSet(false, true)) {
      return;
    }
    Thread.ofVirtual()
        .start(
            () -> {
              ...
              EventThread.getInstance()
                  .enqueue(new RunnableEvent(() -> flushPendingMatches(generation)));
            });
  }
