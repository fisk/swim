...
public class ChatPanelView extends View implements KeyBindingHintProvider {
  ...
  private static final List<String> CODE_KEYWORDS =
      List.of(
          "abstract",
          "boolean",
          "break",
  ...
  public record PromptStyle(
      String inputPrefix,
      String historyPrefix,
      String emptyHint,
      String titleReadyLabel,
      String titlePendingLabel) {
    public static PromptStyle nemo() {
      return new PromptStyle(
          INPUT_PREFIX,
          HISTORY_ME_PREFIX,
          "type a message or :abort",
          " ready ",
          " waiting for Nemo ");
    }
  ...
  private final java.util.function.Function<String, CommandView.CommandMenuState>
      _commandMenuStateProvider;
  ...
  public ChatPanelView(
      Rect bounds, String title, Consumer<String> onSubmit, Consumer<String> onCommand) {
    this(
        bounds,
        title,
        onSubmit,
  ...
  List<String> getDisplayLines() {
    return getDisplayRows().stream().map(DisplayLine::text).toList();
  }
  ...
  public CommandView.CommandMenuState getCommandMenuState() {
    ...
    return new CommandView.CommandMenuState(
        state.visible(), state.prefix(), state.matches(), selection, state.title());
  }
  ...
  private AttributedString renderLine(String line, TextColor background) {
    if (line.startsWith(_promptStyle.historyPrefix())) {
      return renderPromptLine(
          _promptStyle.historyPrefix(),
          line.substring(_promptStyle.historyPrefix().length()),
          UiTheme.CHAT_ME,
          background);
    }
    if (line.startsWith(NEMO_PREFIX)) {
      return renderPromptLine(
          NEMO_PREFIX, line.substring(NEMO_PREFIX.length()), UiTheme.CHAT_NEMO, background);
    }
  ...
  private AttributedString renderLine(DisplayLine line, TextColor background) {
    return switch (line.kind()) {
      ...
      case CODE_HEADER ->
          renderDecoratedLine(line.text(), UiTheme.ACCENT_BLUE, UiTheme.SURFACE_ACCENT);
  ...
  private AttributedString renderCachedCodeLine(DisplayLine line, TextColor background) {
    var key =
        new CodeRenderKey(
            ...
            UiTheme.TEXT_MUTED);
    return _cachedCodeRows.computeIfAbsent(
        key, ignored -> renderCodeLine(line.text(), line.language(), background));
  }

  private AttributedString renderDecoratedLine(
      String line, TextColor textColour, TextColor background) {
    if (line.startsWith(_promptStyle.historyPrefix())) {
      return renderPromptLine(
          _promptStyle.historyPrefix(),
          line.substring(_promptStyle.historyPrefix().length()),
    ...
    if (line.startsWith(NEMO_PREFIX)) {
      return renderPromptLine(
          NEMO_PREFIX,
          line.substring(NEMO_PREFIX.length()),
          UiTheme.CHAT_NEMO,
          textColour,
          background);
    }
  ...
  private AttributedString renderCodeLine(String line, String language, TextColor background) {
    ...
    if (LanguagePluginRegistry.applySnippetColouring(language, result)) {
      return result;
    }
  ...
  private static void formatCodeKeywords(
      AttributedString result, String line, TextColor background) {
  ...
  private static void formatQuotedStrings(
      AttributedString result, String line, TextColor background) {
  ...
  private static void formatCodeComment(
      AttributedString result, String line, TextColor background) {
    ...
    int index =
        slashComment < 0
            ? hashComment
            : hashComment < 0 ? slashComment : Math.min(slashComment, hashComment);
    if (index >= 0) {
  ...
  private AttributedString renderPromptLine(
      String prompt, String text, TextColor promptColour, TextColor background) {
  ...
  List<String> inputLines() {
    int width =
        Math.max(1, getBounds().getSize().getWidth() - (_promptStyle.inputPrefix().length() + 1));
  ...
  public void draw(Rect rect) {
    ...
    if (_contextUsagePercent != null) {
      title.append(
          " ctx " + _contextUsagePercent + "% ",
          contextUsageColour(_contextUsagePercent),
          UiTheme.SURFACE_ACCENT);
    }
    ...
    UiTheme.drawLine(
        graphics, rect.getPoint(), width, title, UiTheme.TEXT_MUTED, UiTheme.SURFACE_ACCENT);
    ...
    for (int i = 0; i < inputHeight && inputStart + i < inputLines.size(); i++) {
      ...
      String prefix =
          i == 0
              ? " " + _promptStyle.inputPrefix()
              : " ".repeat(_promptStyle.inputPrefix().length() + 1);
      ...
      if (_input.length() > 0
          && inputStart + i == cursorLine
          && cursorColumn < inputLine.length()) {
        int caretIndex = Math.max(0, cursorColumn);
        input.format(
            prefix.length() + caretIndex,
            prefix.length() + caretIndex + 1,
            UiTheme.COMMAND_BACKGROUND,
            textColour);
      }
      UiTheme.drawLine(
          graphics,
          Point.create(rect.getPoint().getX(), inputY + i),
          width,
          input,
          UiTheme.TEXT_MUTED,
          UiTheme.COMMAND_BACKGROUND);
