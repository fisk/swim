...
public class NormalMode extends Mode {
  ...
  private void setupBasicResponders() {
    ...
    _fancyCharacterJump =
        new FancyJumpResponder(bufferContext, "g c", FancyJumpResponder.TargetKind.CHARACTER);
    ...
    _rootResponder.addEventResponder(
        ...
        allowed(
            "focus buffer split",
            () -> {
              announceIfUnmoved(window.focusView(Window.Direction.LEFT), "No pane to the left");
            }));
    _rootResponder.addEventResponder(
        "<CTRL>-w j",
        "Panes",
        ...
        allowed(
            "focus buffer split",
            () -> {
              announceIfUnmoved(window.focusView(Window.Direction.DOWN), "No pane below");
            }));
    _rootResponder.addEventResponder(
        "<CTRL>-w k",
        "Panes",
        ...
        allowed(
            "focus buffer split",
            () -> {
              announceIfUnmoved(window.focusView(Window.Direction.UP), "No pane above");
            }));
    _rootResponder.addEventResponder(
        "<CTRL>-w l",
        "Panes",
        ...
        allowed(
            "focus buffer split",
            () -> {
              announceIfUnmoved(window.focusView(Window.Direction.RIGHT), "No pane to the right");
            }));
    _rootResponder.addEventResponder(
        "<CTRL>-w w",
        "Panes",
        ...
        allowed(
            "focus buffer split",
            () -> {
              announceIfUnmoved(window.focusNextView(), "No other pane");
            }));
    _rootResponder.addEventResponder(
        "<CTRL>-w W",
        "Panes",
        ...
        allowed(
            "focus buffer split",
            () -> {
              announceIfUnmoved(window.focusPreviousView(), "No other pane");
            }));
    _rootResponder.addEventResponder(
        "<CTRL>-w q",
        "Panes",
        ...
        allowed(
            "close buffer split",
            () -> {
              announceIfUnmoved(window.closeActiveView(), "Cannot close the last buffer view");
            }));
    _rootResponder.addEventResponder(
        "<CTRL>-w o",
        "Panes",
        ...
        allowed(
            "close buffer splits",
            () -> {
              announceIfUnmoved(window.closeOtherViews(), "No other panes to close");
            }));
    _rootResponder.addEventResponder(
        "<ESC>", "Compile", "hide output", () -> window.hideCompilationOutput());
    _rootResponder.addEventResponder(
        ctrlWMotion(
            ">",
            "wider",
            count -> {
              allow("resize buffer split");
              announceIfUnmoved(
                  window.resizeActiveViewWidth(4 * count), "No vertical split to resize");
            }));
    _rootResponder.addEventResponder(
        ctrlWMotion(
            "<",
    ...
    _rootResponder.addEventResponder(
        ctrlWMotion(
            ...
            count -> {
              allow("resize buffer split");
              announceIfUnmoved(
                  window.resizeActiveViewHeight(2 * count), "No horizontal split to resize");
            }));
    _rootResponder.addEventResponder(
        ctrlWMotion(
            "-",
    ...
    _rootResponder.addEventResponder(
        "<CTRL>-s",
        () -> {
          if (window.blockEditorDriveAction(
              "<CTRL>-s", "sending mail or Slack messages is not allowed")) {
            return;
          }
    ...
    _rootResponder.addEventResponder(
        ...
        "shell",
        () -> {
          startShell(window, ShellTarget.WORKSPACE);
        });
    _rootResponder.addEventResponder(
        "<CTRL>-g c v",
        "Shell",
        "shell in split right",
        "vshell",
        () -> {
          startShell(window, ShellTarget.VERTICAL_SPLIT);
        });
    _rootResponder.addEventResponder(
        "<CTRL>-g c h",
        "Shell",
        "shell in split below",
        "hshell",
        () -> {
          startShell(window, ShellTarget.HORIZONTAL_SPLIT);
        });
    _rootResponder.addEventResponder(
        hinted(
            registerResponder(
                (ignored, register) -> {
                  ...
                  window.selectRegister(register);
                }),
            KeyBindingHint.of("\" <CHAR>", "Registers", "select register")));
    _rootResponder.addEventResponder(
        "g m",
        "Navigation",
        "cycle same-name files",
        "cycle-sibling-file",
        allowed(
            "cycle same-name files",
            () -> announceIfUnmoved(window.cycleSiblingFile(), "No sibling file found")));
    _rootResponder.addEventResponder(
        hinted(
            markJumpResponder("'", true, window),
            KeyBindingHint.of("' <CHAR>", "Marks", "jump to line mark")));
    _rootResponder.addEventResponder(
        hinted(
            markJumpResponder("`", false, window),
            KeyBindingHint.of("` <CHAR>", "Marks", "jump to exact mark")));
    _rootResponder.addEventResponder(
        hinted(macroResponder(window), KeyBindingHint.of("q <CHAR>", "Macros", "record macro")));
    _rootResponder.addEventResponder(
        "g n",
        "Multicursor",
        "add next cursor",
        allowed(
            ...
            () -> {
              announceIfUnmoved(
                  window.addNextCursorForCurrentWord(true), "No next match for multicursor");
            }));
    _rootResponder.addEventResponder(
        "g N",
        "Multicursor",
        ...
        allowed(
            ...
            () -> {
              announceIfUnmoved(
                  window.addNextCursorForCurrentWord(false), "No previous match for multicursor");
            }));
    _rootResponder.addEventResponder(
        "g C",
        "Multicursor",
        "clear extra cursors",
        allowed("multicursor", window::clearAdditionalCursors));
    _rootResponder.addEventResponder(
        "g ]",
        "Diagnostics",
        ...
        allowed(
            ...
            () -> {
              window.navigateDiagnostic(true, false);
            }));
    _rootResponder.addEventResponder(
        "g [",
        "Diagnostics",
        ...
        allowed(
            "diagnostic navigation",
            () -> {
              window.navigateDiagnostic(false, false);
            }));
    _rootResponder.addEventResponder(
        "g }",
        "Diagnostics",
        ...
        allowed(
            "diagnostic navigation",
            () -> {
              window.navigateDiagnostic(true, true);
            }));
    _rootResponder.addEventResponder(
        "g {",
        "Diagnostics",
        ...
        allowed(
            "diagnostic navigation",
            () -> {
              window.navigateDiagnostic(false, true);
            }));
    _rootResponder.addEventResponder(
        "g x",
        "Diagnostics",
        ...
        allowed(
            "diagnostic popup",
            () -> {
              window.showDiagnosticsForCurrentLine(true);
            }));
    _rootResponder.addEventResponder(
        "c ]",
        "Conflicts",
    ...
    _rootResponder.addEventResponder(
        ...
        allowed(
            "code actions",
            () -> {
              window.showCodeActionsForCurrentLine();
            }));
    _rootResponder.addEventResponder(
        hinted(
            prefixCharacterResponder(
                "@",
                (ignored, register) -> {
                  if (window.blockEditorDriveAction(
                      "macro playback", "macros are outside the editor-control sandbox")) {
                    return;
                  }
                  if (register == '@') {
    ...
    _rootResponder.addEventResponder(
        new MotionResponder(
            ...
            count -> {
              allow("jump navigation");
              repeat(count, window::jumpForward);
            }));
    _rootResponder.addEventResponder(
        hinted(
            foldCreateResponder(window),
            ...
            () -> {
              announceIfUnmoved(
                  window
                      .getBufferContext()
                      .getBuffer()
                      .closeFoldAt(window.getBufferContext().getBuffer().getCursor().getPosition()),
                  "No fold at cursor");
            }));
    _rootResponder.addEventResponder(
        "z o",
            ...
            () -> {
              announceIfUnmoved(
                  window
                      .getBufferContext()
                      .getBuffer()
                      .openFoldAt(window.getBufferContext().getBuffer().getCursor().getPosition()),
                  "No fold at cursor");
            }));
    _rootResponder.addEventResponder(
        "z v",
    ...
    _rootResponder.addEventResponder(
        ...
        allowed(
            "fold",
            () -> {
              window.getBufferContext().getBuffer().deleteAllFolds();
            }));
    _rootResponder.addEventResponder(
        new MotionResponder(
            "z j",
            ...
            "next fold",
            count -> {
              allow("fold navigation");
              announceIfUnmoved(goFoldStart(window, true, count), "No next fold");
            }));
    _rootResponder.addEventResponder(
        new MotionResponder(
            "z k",
            ...
            count -> {
              allow("fold navigation");
              announceIfUnmoved(goFoldStart(window, false, count), "No previous fold");
            }));
    _rootResponder.addEventResponder(
        "z M",
        "Folds",
        ...
        allowed(
            "fold",
            () -> {
              window.getBufferContext().getBuffer().closeAllFolds();
            }));
    _rootResponder.addEventResponder(
        "z R",
        "Folds",
        ...
        allowed(
            "fold",
            () -> {
              window.getBufferContext().getBuffer().openAllFolds();
            }));
    _rootResponder.addEventResponder(
        "z t",
        "Viewport",
        ...
        allowed(
            ...
            () -> {
              window
                  .getBufferContext()
                  .getBufferView()
                  .alignCursorLine(BufferView.ViewportAnchor.TOP);
            }));
    _rootResponder.addEventResponder(
        "z z",
        "Viewport",
        "cursor line to middle",
        allowed(
            "scroll buffer",
            () -> {
              window
                  .getBufferContext()
                  .getBufferView()
                  .alignCursorLine(BufferView.ViewportAnchor.MIDDLE);
            }));
    _rootResponder.addEventResponder(
        "z b",
        "Viewport",
        ...
        allowed(
            ...
            () -> {
              window
                  .getBufferContext()
                  .getBufferView()
                  .alignCursorLine(BufferView.ViewportAnchor.BOTTOM);
            }));
    _rootResponder.addEventResponder(
        "u",
        "Editing",
    ...
    _rootResponder.addEventResponder(
        ...
        () -> {
          ...
          activeBuffer.yankRange(
              range.getStart(), range.getEnd(), true, window.consumeSelectedRegister());
        });
    _rootResponder.addEventResponder(
        "J",
        "Editing",
    ...
    _rootResponder.addEventResponder(
        ...
        () -> {
          ...
          activeBuffer.changeRange(
              range.getStart(), range.getEnd(), true, window.consumeSelectedRegister());
          window.switchToMode(window.getInputMode());
        });
    _rootResponder.addEventResponder(
        hinted(
            replaceCharacterResponder(window),
    ...
    _rootResponder.addEventResponder(
        ...
        () -> {
          if (window.blockEditorDriveAction(
              "debug breakpoint", "debugger actions are outside the editor-control sandbox")) {
            return;
          }
          try {
            ...
            window
                .getCommandView()
                .setMessage(
                    e.getMessage() == null ? "Failed to toggle breakpoint" : e.getMessage());
          }
        });
    _rootResponder.addEventResponder(
        ":",
    ...
    _rootResponder.addEventResponder(
        ...
        () -> {
          if (window.blockEditorDriveAction(
              "Nemo chat", "opening Nemo from editor control is not allowed")) {
            return;
          }
    ...
    _rootResponder.addEventResponder(
        "g *",
        "Search",
        "search current word fragment forward",
        () -> {
    ...
    _rootResponder.addEventResponder(
        "g #",
        "Search",
        "search current word fragment backward",
        () -> {
  ...
  private void installLeaderMoveBindings(Window window, String leader) {
    _rootResponder.addEventResponder(
        new MotionResponder(leader + " h", count -> indentCurrentLines(window, -1, count)));
    _rootResponder.addEventResponder(
        new MotionResponder(leader + " J", count -> moveCurrentLines(window, 1, count)));
    _rootResponder.addEventResponder(
        new MotionResponder(leader + " k", count -> moveCurrentLines(window, -1, count)));
    _rootResponder.addEventResponder(
        new MotionResponder(leader + " l", count -> indentCurrentLines(window, 1, count)));
  ...
  private void installLeaderWorkspaceBindings(Window window, String leader) {
    ...
    _rootResponder.addEventResponder(
        ...
        () -> {
          if (window.blockEditorDriveAction(
              "Todo workspace", "Todo is outside the editor-control sandbox")) {
            return;
          }
    ...
    _rootResponder.addEventResponder(
        ...
        () -> {
          if (window.blockEditorDriveAction(
              "shell panel", "opening shell input through drive_editor is not allowed")) {
            return;
          }
  ...
  private static final class PluginKeyBindingRegistryResponder
    ...
    private void reportPluginKeyBindingConflicts(List<SwimPluginKeyBindingDescriptor> bindings) {
      for (var conflict :
          findPluginKeyBindingConflicts(
              bindings, _rootResponder.keyBindingHintsExcluding(_pluginLayer))) {
  ...
  static List<KeyBindingConflict> findPluginKeyBindingConflicts(
      List<SwimPluginKeyBindingDescriptor> pluginBindings, List<KeyBindingHint> normalModeHints) {
    ...
    for (var binding : pluginBindings) {
      ...
      for (var hint : normalModeHints) {
        ...
        if (isStrictKeyPrefix(hint.key(), binding.key())) {
          conflicts.add(
              new KeyBindingConflict(
                  binding.pluginId(), binding.key(), hint.key(), hint.summary()));
        }
      }
    }
  ...
  private static final class PluginKeyBindingResponder
      implements EventResponder, KeyBindingHintProvider {
    private final Window _window;
    ...
    private PluginKeyBindingResponder(Window window, SwimPluginKeyBindingDescriptor binding) {
      ...
      _binding = binding;
      _responder =
          new TextEventResponder(
              binding.key(),
              this::run,
              KeyBindingHint.of(
                  binding.key(), binding.group(), binding.summary(), binding.commandName()));
    }
  ...
  private void moveCurrentLines(Window window, int delta, int lineCount) {
    ...
    if (result == null) {
      window
          .getCommandView()
          .setMessage(delta > 0 ? "Cannot move lines down" : "Cannot move lines up");
      return;
    }
  ...
  private EventResponder replaceCharacterResponder(Window window) {
    return new EventResponder() {
      ...
      public Response processEvent(KeyStrokes events) {
        ...
        if (character.getKeyType() != KeyType.Character
            || character.isCtrlDown()
            || character.isAltDown()) {
          return Response.NO;
        }
  ...
  private EventResponder characterFindResponder(Window window) {
    return new EventResponder() {
      ...
      public Response processEvent(KeyStrokes events) {
        ...
        if (!isCharacter(stroke, 'f')
            && !isCharacter(stroke, 'F')
            && !isCharacter(stroke, 't')
            && !isCharacter(stroke, 'T')) {
          return Response.NO;
        }
        _count = count.count();
      ...
      public void respond() {
        ...
        var buffer = window.getBufferContext().getBuffer();
        int position =
            buffer.findCharacterOnLine(
                buffer.getCursor().getPosition(), _character, forward, _till, _count);
        if (position >= 0) {
  ...
  private EventResponder operatorResponder(Window window) {
    return new EventResponder() {
      ...
      public Response processEvent(KeyStrokes events) {
        ...
        var motion =
            parseOperatorMotion(
                window, sequence, operator.index(), operatorCount.count(), operator.operator());
        if (motion.status() != ParseStatus.YES) {
          return motion.status().response();
        }
        if (sequence.size() != motion.index()) {
  ...
  private void applyOperator(Window window, OperatorExecution execution) {
    ...
    switch (execution.operator()) {
      ...
      case TOGGLE_CASE -> {
        window.allowEditorDriveAction("buffer edit");
        buffer.transformRange(
            range.getStart(), range.getEnd(), execution.lineWise(), NormalMode::toggleCase);
        buffer.getUndoLog().commit();
      }
    }
  }

  private OperatorMotionParse parseOperatorMotion(
      Window window, List<KeyStroke> sequence, int index, int operatorCount, Operator operator) {
    ...
    if (operator != null && isRepeatedOperator(operator, first, sequence, motionIndex)) {
      ...
      return OperatorMotionParse.yes(
          motionIndex + repeatedOperatorLength(operator, sequence, motionIndex), range, true);
    }
    ...
    if (isCharacter(first, 'i') || isCharacter(first, 'a')) {
      ...
      var range =
          buffer.textObjectRange(
              Character.toString(object.getCharacter()), isCharacter(first, 'a'));
    ...
    if (isCharacter(first, 'g')) {
      ...
      if (isCharacter(second, 'e') || isCharacter(second, 'E')) {
        ...
        return OperatorMotionParse.yes(
            motionIndex + 2,
            Range.create(Math.min(start, target), Math.max(start, target) + 1),
            false);
      }
      ...
      if (isCharacter(second, '*') || isCharacter(second, '#')) {
        return currentWordSearchMotion(
            buffer, start, count, !isCharacter(second, '#'), false, motionIndex + 2);
      }
      return OperatorMotionParse.no();
    }
    ...
    if (isCharacter(first, '*') || isCharacter(first, '#')) {
      return currentWordSearchMotion(
          buffer, start, count, !isCharacter(first, '#'), true, motionIndex + 1);
    }
    ...
    if (isCharacter(first, '\'') || isCharacter(first, '`')) {
      ...
      if (isCharacter(first, '\'')) {
        return OperatorMotionParse.yes(
            motionIndex + 2,
            buffer.lineRangeForPositions(start, buffer.getLineStartPosition(target)),
            true);
      }
      return OperatorMotionParse.yes(
          motionIndex + 2, Range.create(Math.min(start, target), Math.max(start, target)), false);
    }
    ...
    if (isCharacter(first, 'n') || isCharacter(first, 'N')) {
      ...
      return OperatorMotionParse.yes(
          motionIndex + 1, Range.create(Math.min(start, target), Math.max(start, target)), false);
    }
    ...
    if (isCharacter(first, '/') || isCharacter(first, '?')) {
      ...
      int target =
          findSearchTarget(buffer.getString(), start, search.pattern(), isCharacter(first, '/'));
      if (target < 0) {
        return OperatorMotionParse.no();
      }
    ...
    if (isCharacter(first, ';') || isCharacter(first, ',')) {
      ...
      int found =
          buffer.findCharacterOnLine(start, _lastFindCharacter, forward, _lastFindTill, count);
      if (found < 0) {
        return OperatorMotionParse.no();
      }
      int end = forward ? found + 1 : found;
      return OperatorMotionParse.yes(
          motionIndex + 1, Range.create(Math.min(start, found), Math.max(start, end)), false);
    }

    if (isCharacter(first, 'f')
        || isCharacter(first, 'F')
        || isCharacter(first, 't')
        || isCharacter(first, 'T')) {
      if (sequence.size() == motionIndex + 1) {
      ...
      int end = forward ? found + 1 : found;
      return OperatorMotionParse.yes(
          motionIndex + 2, Range.create(Math.min(start, begin), Math.max(start, end)), false);
    }
    ...
    return OperatorMotionParse.yes(
        motionIndex + target.consumed(), target.range(), target.lineWise());
  }

  private MotionTarget motionTarget(
      org.fisk.swim.text.Buffer buffer,
      KeyStroke stroke,
      int start,
      int count,
    ...
    if (isCharacter(stroke, 'l')) {
      return MotionTarget.character(
          Range.create(start, Math.min(buffer.getLength(), start + count)), 1);
    }
    if (isCharacter(stroke, 'j')) {
      int line = Math.min(buffer.getLineCount() - 1, buffer.getLineIndexAt(start) + count);
      return MotionTarget.line(
          buffer.lineRangeForPositions(start, buffer.getLineStartByIndex(line)), 1);
    }
    ...
    if (isCharacter(stroke, '+') || isCharacter(stroke, '-')) {
      ...
      int line =
          Math.max(
              0,
              Math.min(
                  buffer.getLineCount() - 1, buffer.getLineIndexAt(start) + direction * count));
      return MotionTarget.line(
          buffer.lineRangeForPositions(
              start, firstNonBlankPosition(buffer, buffer.getLineStartByIndex(line))),
          1);
    }
    if (isCharacter(stroke, '^') || isCharacter(stroke, '_')) {
      ...
      while (target < lineEnd
          && (buffer.getCharacter(target).equals(" ")
              || buffer.getCharacter(target).equals("\t"))) {
        target++;
      }
      return MotionTarget.character(
    ...
    if (isCharacter(stroke, '$')) {
      return MotionTarget.character(
          Range.create(start, buffer.getLineEndPosition(start, false)), 1);
    }
    ...
    if (isCharacter(stroke, 'e') || isCharacter(stroke, 'E')) {
      ...
      return MotionTarget.character(
          Range.create(start, Math.min(buffer.getLength(), target + 1)), 1);
    }
    ...
    if (isCharacter(stroke, '%')) {
      ...
      return MotionTarget.character(
          Range.create(
              Math.min(start, target), Math.min(buffer.getLength(), Math.max(start, target) + 1)),
          1);
    }
    if (isCharacter(stroke, 'G')) {
      int line =
          hasExplicitCount
              ? Math.min(buffer.getLineCount() - 1, count - 1)
  ...
  private static int findSearchTarget(
      String text, int start, Pattern pattern, boolean forward, int count) {
    int target = start;
  ...
  private static int findSearchTargetOnce(
      String text, int start, Pattern pattern, boolean forward) {
  ...
  private static OperatorMotionParse currentWordSearchMotion(
      org.fisk.swim.text.Buffer buffer,
      int start,
      int count,
      boolean forward,
    ...
    int target =
        findSearchTarget(buffer.getString(), start, Pattern.compile(expression), forward, count);
    if (target < 0) {
      return OperatorMotionParse.no();
    }
    return OperatorMotionParse.yes(
        index, Range.create(Math.min(start, target), Math.max(start, target)), false);
  }
  ...
  private static int firstNonBlankPosition(org.fisk.swim.text.Buffer buffer, int position) {
    ...
    int target = position;
    while (target < end
        && (buffer.getCharacter(target).equals(" ") || buffer.getCharacter(target).equals("\t"))) {
      target++;
    }
    return target;
  }
  ...
  private static boolean isRepeatedOperator(
      Operator operator, KeyStroke first, List<KeyStroke> sequence, int index) {
    return switch (operator) {
      ...
      case UPPER ->
          sequence.size() > index + 1
                  && isCharacter(first, 'g')
                  && isCharacter(sequence.get(index + 1), 'U')
              || isCharacter(first, 'U');
      case LOWER ->
          sequence.size() > index + 1
                  && isCharacter(first, 'g')
      ...
      case TOGGLE_CASE ->
          sequence.size() > index + 1
                  && isCharacter(first, 'g')
                  && isCharacter(sequence.get(index + 1), '~')
              || isCharacter(first, '~');
    };
  }

  private static int repeatedOperatorLength(
      Operator operator, List<KeyStroke> sequence, int index) {
    return switch (operator) {
      case UPPER, LOWER, TOGGLE_CASE ->
          index < sequence.size() && isCharacter(sequence.get(index), 'g') ? 2 : 1;
      default -> 1;
    };
  }
  ...
  private record OperatorExecution(
      Operator operator, Range range, boolean lineWise, List<RecordedKey> keys) {}
  ...
  private static EventResponder prefixCharacterResponder(
      String prefix, BiConsumer<Integer, Character> responder) {
    return new EventResponder() {
      private int _count;
      private Character _character;
      ...
      public Response processEvent(KeyStrokes events) {
        ...
        if (argument.getKeyType() != org.fisk.swim.event.KeyType.Character
            || argument.isCtrlDown()
            || argument.isAltDown()) {
          return Response.NO;
        }
  ...
  private static EventResponder markJumpResponder(String prefix, boolean lineWise, Window window) {
    return prefixCharacterResponder(
        prefix,
        (ignored, mark) -> {
  ...
  private static EventResponder macroResponder(Window window) {
    return new EventResponder() {
      ...
      public Response processEvent(KeyStrokes events) {
        ...
        if (first.getKeyType() != org.fisk.swim.event.KeyType.Character
            || first.getCharacter() != 'q'
            || first.isCtrlDown()
            || first.isAltDown()) {
          return Response.NO;
        }
        ...
        if (second.getKeyType() != org.fisk.swim.event.KeyType.Character
            || second.isCtrlDown()
            || second.isAltDown()) {
          return Response.NO;
        }
      ...
      public void respond() {
        if (window.blockEditorDriveAction(
            "macro", "macros are outside the editor-control sandbox")) {
          return;
        }
        if (_stop) {
  ...
  private static EventResponder ctrlWMotion(
      String motion, String summary, MotionResponder.Responder responder) {
    EventResponder delegate =
        new EventResponder() {
  ...
  private void installTextObjectResponder(
      Window window, String pattern, String object, boolean around, TextObjectOperator operator) {
    _rootResponder.addEventResponder(
        pattern,
        "Editing",
        textObjectSummary(object, around),
  ...
  private void applyTextObject(
    ...
    switch (operator) {
      case DELETE -> {
        ...
        buffer.deleteRange(
            range.getStart(), range.getEnd(), false, window.consumeSelectedRegister());
        buffer.getUndoLog().commit();
        window.commitRepeatRecording();
      }
      case CHANGE -> {
