...
public class AttributedString {
  ...
  public static record FormatRange(
      int start, int end, TextColor foregroundColour, TextColor backgroundColour) {}
  ...
  public static AttributedString create(
      String string, TextColor foregroundColour, TextColor backgroundColour) {
  ...
  public void append(String string, TextColor foregroundColour, TextColor backgroundColour) {
    _fragments.add(
        new AttributedStringFragment(string, new AttributeSet(foregroundColour, backgroundColour)));
  ...
  public AttributedString onClick(int start, int end, ClickHandler handler) {
    if (start < 0 || end < start || end > _length) {
      throw new IllegalArgumentException(
          "Click range out of bounds: " + start + ", " + end + " length: " + _length);
    }
  ...
  public AttributedString slice(int start, int end) {
    if (start < 0 || end < start || end > _length) {
      throw new IllegalArgumentException(
          "Slice out of bounds: " + start + ", " + end + " length: " + _length);
    }
    ...
    for (var fragment : _fragments) {
      ...
      result._length +=
          result.formatFragmentRange(
              Range.create(start, end), fragmentRange, fragment._attributes, fragment._string);
  ...
  private int formatFragmentRange(
      Range range, Range fragmentRange, AttributeSet attrs, String fragmentStr) {
    ...
    var str =
        fragmentStr.substring(
            range.getStart() - fragmentRange.getStart(), range.getEnd() - fragmentRange.getStart());
  ...
  public void format(int start, int end, TextColor foregroundColour, TextColor backgroundColour) {
    ...
    for (var fragment : oldFragments) {
      ...
      if (currentX + fragmentLength <= start || currentX >= end) {
        ...
        formatFragmentRange(
            Range.create(currentX, start), fragmentRange, fragment._attributes, fragment._string);
        formatFragmentRange(Range.create(start, end), fragmentRange, newAttr, fragment._string);
        formatFragmentRange(
            Range.create(end, currentX + fragmentLength),
            fragmentRange,
  ...
  public void format(List<FormatRange> ranges) {
    ...
    for (var range : ranges) {
      ...
      if (end > start) {
        validRanges.add(
            new FormatRange(start, end, range.foregroundColour(), range.backgroundColour()));
      }
    }
    ...
    for (var fragment : _fragments) {
      ...
      Arrays.fill(
          ...
          currentX,
          currentX + fragmentLength,
          fragment._attributes._foregroundColour);
      Arrays.fill(
          backgroundColours,
          currentX,
          currentX + fragmentLength,
          fragment._attributes._backgroundColour);
      currentX += fragmentLength;
    }
    ...
    while (start < _length) {
      ...
      _fragments.add(
          new AttributedStringFragment(
              string.substring(start, end), new AttributeSet(foreground, background)));
      start = end;
    }
    resetCharacterLookupCache();
  }
  ...
  public void insert(
      String str, int position, TextColor foregroundColour, TextColor backgroundColour) {
    _log.debug("Inserting " + str + " at " + position);
    if (position > _length || position < 0) {
      throw new IllegalArgumentException(
          "Insert out of bounds: " + position + " length: " + _length);
    }
    var newAttr = new AttributeSet(foregroundColour, backgroundColour);
    ...
    for (int index = 0; index < _fragments.size(); index++) {
      ...
      if (position >= currentX && position < currentX + fragmentLength) {
        ...
        if (!postString.isEmpty()) {
          _fragments.add(index, new AttributedStringFragment(postString, fragment._attributes));
        }
        _fragments.add(index, new AttributedStringFragment(str, newAttr));
        if (!preString.isEmpty()) {
          _fragments.add(index, new AttributedStringFragment(preString, fragment._attributes));
        }
        _length += str.length();
  ...
  private void updateClickRangesAfterInsert(
      List<ClickRange> oldClickRanges, int position, int length) {
  ...
  public void remove(int startPosition, int endPosition) {
    ...
    for (var fragment : oldFragments) {
      ...
      currentX -= fragmentLength;
      var fragmentRange = Range.create(currentX, currentX + fragmentLength);
      if (currentX + fragmentLength <= startPosition || currentX >= endPosition) {
        _fragments.add(fragment);
      } else {
        formatFragmentRange(
            Range.create(endPosition, currentX + fragmentLength),
            fragmentRange,
            fragment._attributes,
            fragment._string);
        formatFragmentRange(
            Range.create(currentX, startPosition),
            fragmentRange,
            fragment._attributes,
            fragment._string);
      }
  ...
  public AttributedString getCharacter(int position) {
    ...
    return AttributedString.create(
        fragment._string.substring(position - fragmentStart, position - fragmentStart + 1),
        attributes._foregroundColour,
        attributes._backgroundColour);
  }
  ...
  public AttributeSet attributesAt(int position) {
    ...
    for (int i = fragmentIndex; i < _fragments.size(); ++i) {
      ...
      int fragmentLength = fragment._string.length();
      if (position >= currentX && position + 1 <= currentX + fragmentLength) {
  ...
  public void drawAt(Point point, TerminalGraphics graphics) {
    ...
    for (var fragment : _fragments) {
      ...
      graphics.putString(
          ...
          fragment._string,
          new AnsiStyle(
              AnsiColour.fromTextColor(attributes._foregroundColour),
              AnsiColour.fromTextColor(attributes._backgroundColour),
              false,
              false,
              false));
  ...
  private void registerRenderedClickRanges(Point point) {
    ...
    synchronized (CLICK_MAP_LOCK) {
      for (ClickRange range : _clickRanges) {
        _renderedClickRanges.add(
            new RenderedClickRange(
                point.getX() + range.start(),
                point.getX() + range.end(),
                point.getY(),
                range.handler()));
      }
    }
