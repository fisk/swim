...
public class TextLayout {
  ...
  private record LayoutSnapshot(
      int version,
      int width,
      ArrayList<Line> physicalLines,
      TreeMap<Integer, Line> physicalLineAtPosition,
      ArrayList<Line> logicalLines,
  ...
  private ArrayList<Line> rebuildPhysicalSuffix(Line startLine, int maximumCharacters) {
    ...
    result.addAll(
        buildPhysicalLinesFrom(
            startLine.getStartPosition(), startLine.getY(), previous, maximumCharacters));
    return result;
  }

  private ArrayList<Line> buildPhysicalLinesFrom(
      int startPosition, int startY, Line previous, int maximumCharacters) {
    var string = _bufferContext.getBuffer().getString();
    ...
    Line line =
        new Line(y, position, previous, position > 0 && string.charAt(position - 1) == '\n');
    if (previous != null) {
      previous.setNext(line);
    }
    result.add(line);
    int endPosition =
        maximumCharacters == Integer.MAX_VALUE
            ? string.length()
  ...
  private ArrayList<Line> rebuildLogicalSuffix(Line startLine, int width, int maximumCharacters) {
    ...
    Line previous = result.isEmpty() ? null : result.get(result.size() - 1);
    result.addAll(
        buildLogicalLinesFrom(
            startLine.getStartPosition(), startLine.getY(), previous, width, maximumCharacters));
    return result;
  }

  private ArrayList<Line> buildLogicalLinesFrom(
      int startPosition, int startY, Line previous, int width, int maximumCharacters) {
    var string = _bufferContext.getBuffer().getString();
    int position = Math.max(0, Math.min(startPosition, string.length()));
    int y = startY;
    int x = 0;
    ...
    Thread.ofVirtual()
        .start(
            () -> {
              ...
              if (generation != _asyncLayoutGeneration.get()) {
                return;
              }
              EventThread.getInstance()
                  .enqueue(new RunnableEvent(() -> captureAsyncLayout(generation, version, width)));
            });
  }
  ...
  private void captureAsyncLayout(long generation, int version, int width) {
    if (generation != _asyncLayoutGeneration.get()
        || version != _bufferContext.getBuffer().getVersion()) {
      return;
    }
    ...
    Thread.ofVirtual()
        .start(
            () -> {
              ...
              EventThread.getInstance()
                  .enqueue(new RunnableEvent(() -> applySnapshot(generation, snapshot)));
            });
  }

  private void applySnapshot(long generation, LayoutSnapshot snapshot) {
    if (generation != _asyncLayoutGeneration.get()
        || snapshot.version() != _bufferContext.getBuffer().getVersion()
        || snapshot.width() != currentWidth()
        || hasCollapsedFolds()) {
      return;
    }
  ...
  private static LayoutSnapshot buildSnapshot(String text, int version, int width) {
    ...
    return new LayoutSnapshot(
        version, width, physical.lines(), physical.map(), logical.lines(), logical.map());
  }
  ...
  private static final class SnapshotBuilder {
    ...
    private Lines lines(boolean wrap) {
      ...
      for (int position = 0; position < _text.length(); position++) {
        ...
        if (character == '\n') {
          if (!wrap) {
            line.getGlyphs().add(new Glyph(x, y, position, "\n"));
          }
          Line next = new Line(++y, position + 1, line, true);
          line.setNext(next);
          line = next;
  ...
  public Stream<Glyph> getGlyphs() {
    return getVisibleLogicalLines().stream()
        .map((line) -> line.getGlyphs())
