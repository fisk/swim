...
public sealed interface TextColor permits TextColor.RGB, TextColor.Indexed, TextColor.ANSI {
    ...
    record Indexed(int index) implements TextColor {
        public Indexed {
            if (index < 0 || index > 255) {
                throw new IllegalArgumentException("ANSI colour index must be in [0,255]");
            }
        }
        ...
        public static Indexed fromRGB(int red, int green, int blue) {
            return new Indexed(16 + 36 * Math.round(red / 51f) + 6 * Math.round(green / 51f)
                    + Math.round(blue / 51f));
        }
    }
    ...
    enum ANSI implements TextColor {
        ...
        ANSI(int red, int green, int blue) {
            this.red = red;
            this.green = green;
            this.blue = blue;
        }
        ...
        public boolean isBright() {
            return ordinal() >= BLACK_BRIGHT.ordinal();
        }
    }
    ...
    final class Factory {
        ...
        public static TextColor fromString(String value) {
            ...
            if (colour.startsWith("#") && colour.length() == 7) {
                return new RGB(Integer.parseInt(colour.substring(1, 3), 16),
                        Integer.parseInt(colour.substring(3, 5), 16), Integer.parseInt(colour.substring(5, 7), 16));
            }
            try {
                return ANSI.valueOf(colour.toUpperCase(Locale.ROOT));
            } catch (IllegalArgumentException e) {
                throw new IllegalArgumentException("Unknown terminal colour: " + value, e);
            }
        }
    }
    ...
    private static void validate(int value) {
        if (value < 0 || value > 255) {
            throw new IllegalArgumentException("RGB component must be in [0,255]");
        }
    }
