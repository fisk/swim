...
final class NemoConditionalEdit {
  ...
  /**
   * The source changed after Nemo inspected it. This is an expected optimistic-concurrency outcome,
   * rather than a broken edit operation.
   */
  ...
  static Change replaceLines(
      String text, int startLine, int endLine, String expected, String replacement)
      throws IOException {
    if (startLine < 1 || endLine < startLine) {
      throw new IOException("Invalid line range " + startLine + "-" + endLine);
    }
    List<String> lines = splitLines(text);
    if (endLine > lines.size()) {
      throw new IOException("Line range exceeds file length (" + lines.size() + ")");
    }
    String actual = String.join("\n", lines.subList(startLine - 1, endLine));
    if (!actual.equals(expected)) {
      throw new PreconditionFailedException(
          "expected text no longer matches lines " + startLine + "-" + endLine);
    }
  ...
  static Change replaceFunctionBody(
      String text, String function, String expectedBody, String replacement) throws IOException {
    if (function == null || function.isBlank()) {
      throw new IOException("function is required");
    }
    int match = -1;
    int open = -1;
    int from = 0;
    while ((from = text.indexOf(function, from)) >= 0) {
      ...
      if (candidate >= 0 && (semicolon < 0 || candidate < semicolon)) {
        if (match >= 0) {
          throw new IOException("Function anchor is ambiguous: " + function);
        }
        match = from;
        open = candidate;
      }
    ...
    if (match < 0) {
      throw new IOException("Function anchor not found: " + function);
    }
    int close = matchingBrace(text, open);
    if (close < 0) {
      throw new IOException("Function anchor has an unmatched body brace: " + function);
    }
  ...
  private static List<String> splitReplacement(String replacement) {
    return List.of(replacement.split("\\n", -1));
  }
