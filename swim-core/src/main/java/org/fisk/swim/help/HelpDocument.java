        ...
        return List.of(
                chapter("start", "Start Here",
                        ...
                        section("Saving and leaving",
                                ":w writes the current buffer to disk. :w path writes the current buffer to a specific path. :q follows Vim's window semantics: it closes the current frame first, closes the current tab when that was the last frame in the tab, and exits SWIM only when the last tab is gone. :q! follows the same close path without save checks. If you are reading a workspace such as help, q returns to your previous workspace without exiting the editor.",
                                ":w\n:w draft.txt\n:q"),
                ...
                chapter("files", "Files, Buffers, and Panes",
                        ...
                        section(":e and startup files",
                                ":e path opens an existing file or creates it when possible. Launching with no file opens an editable *scratch* buffer. Launching with several file paths opens the first file and keeps the others ready as buffers.",
                        ...
                        section("Buffer navigation",
                                ...
                                ":ls\n:b 2\n:b Main.java\n:bnext"),
                        section("Tabs and project buffer groups",
                                "The bottom bar shows stable tmux-style tabs. A project buffer tab displays the project name, while application and shell tabs display their app names. Tabs can also be renamed when you want a short task-oriented label. The top context bar stays local to the active project buffer group: it shows the MRU buffers for that project rather than every app, shell, or unrelated project tab. This keeps several projects open at once without mixing their buffer lists.",
                                "<CTRL>-b 0\n<CTRL>-b ,\n:tab-rename review\n:buffers"),
                        section("Splits",
                        ...
                        section("Moving focus and resizing",
                                ...
                                ":focus right\n<CTRL>-w h\n<CTRL>-w ="),
                        section("Tmux-style prefix",
                                "Ctrl-b opens SWIM's tmux-style prefix layer. SWIM maps tmux windows to editor tabs and tmux panes to editor frames, so Ctrl-b c creates a shell tab, Ctrl-b n and Ctrl-b p move through tabs, Ctrl-b 0 through Ctrl-b 9 jump by stable tab index, Ctrl-b ' prompts for any tab number followed by Enter, Ctrl-b w opens a chooser, Ctrl-b , renames the current tab, Ctrl-b . moves the current tab to a typed index, Ctrl-b < and Ctrl-b > swap the current tab left or right, and Ctrl-b & closes the current tab. Closing the last tab exits the editor session. Ctrl-b d detaches the client while leaving the server-side session running, and Ctrl-b s opens the server-session chooser. Frame commands follow tmux too: Ctrl-b \" splits below, Ctrl-b % splits right, Ctrl-b arrow keys focus neighboring frames, Ctrl-b o cycles frames, Ctrl-b ; returns to the last frame, Ctrl-b x closes the frame, Alt-arrows resize, and Ctrl-b Space equalizes sizes. Ctrl-b : opens the command prompt and Ctrl-b ? shows a compact prefix reference.",
                                "<CTRL>-b c\n<CTRL>-b ' 10 <ENTER>\n<CTRL>-b ,\n<CTRL>-b . 2 <ENTER>\n<CTRL>-b >\n<CTRL>-b d\n<CTRL>-b s\n<CTRL>-b %\n<CTRL>-b <RIGHT>\n<CTRL>-b :"),
                        section("Closing panes and workspaces",
                                ":close closes the active pane when more than one pane exists, and :only keeps only the active pane. Full workspaces such as help, shell, and Nemo can use q to return to the previous workspace without deleting the underlying buffer state. Todo uses q or Esc to close its tab.",
                                ":close\n:only\nq")),
                chapter("commands", "Command Line Reference",
                        ...
                        section("Project search and location lists",
                                ":grep text searches project contents and opens matching results. While you type a :grep command, the project search panel previews results for each character so you can refine the query before pressing Enter. g \\ opens the same search with an editable-results Enter action: edit the text after each path:line prefix to write through to that source file. The result list uses the project-local .swim ignore rules, with gitignore-style patterns such as build/, *.log, and !keep.log. Existing .swim settings such as compile_commands are ignored by the search filter. :compile prompts for a project build command; :compile-file runs the exact compile_commands.json entry for the current translation unit. :compile-log, :compile-follow [on|off], and :compile-status manage the live read-only build buffer. :gclog opens the live read-only rotating SWIM GC log. :copen, :cnext, and :cprev walk the quickfix list. :lgrep searches only the current buffer into a location list, and :lopen, :lnext, and :lprev navigate it.",
                                ":grep parse error\n.swim:\n  build/\n  *.log\n  !keep.log\n:cnext\n:lgrep TODO\n:lopen"),
                ...
                chapter("diagnostics", "Diagnostics and Code Intelligence",
                        ...
                        section("Project diagnostic navigation",
                                ...
                                "g]\ng}")),
                chapter("workspaces", "Todo, Shell, Sessions, and Plugin Panels",
                        "Core workspaces and the panel hooks that plugins can extend.",
                        section("Todo",
                                ":todo opens the Todo workspace. Ctrl-t opens quick capture from any screen, Enter adds the item to the Inbox, and Esc cancels. Inside Todo, n creates an item, p assigns a project, g edits tags, c or Enter toggles completion, x deletes, and q or Esc closes the Todo tab.",
                                "<CTRL>-t\n:todo"),
                        section("Shell",
                        ...
                        section("Plugin panels",
                                "Plugins can add full-screen workspaces, side panels, key bindings, and their own help chapters. When a plugin is installed, look for plugin-specific topics in :help, such as Git workflow, language-server support, mail, Slack, tree view, debugger, or JFR chapters. :jfr starts a live recording of SWIM when needed; :jfr file.jfr opens an existing recording; and :jfr before.jfr,after.jfr compares recordings by CPU and committed heap memory.",
                                ":help\n:help git\n:jfr\n:jfr before.jfr,after.jfr"),
                        section("Server sessions",
                                ":sessions lists the live swim instances managed by the background session server. Select one and press Enter to move the current terminal client to that instance. :session name switches directly to a named instance, creating it on demand with an empty launch argument list. From the shell, swim --attach name starts the client already attached to that named session instead of interpreting the name as a file. The client sends its current working directory to the server, and the server starts a newly created app JVM from that directory when it still exists, so relative paths behave as if SWIM had launched in the foreground. If a session is wedged, :session-kill name or swim --kill-session name asks the server to disconnect its client and terminate the session process tree so its threads and class loader can be released.",
                ...
                chapter("nemo", "Nemo Assistant",
                        ...
                        section("Starting Nemo",
                                "Press ! to return to the Nemo popup conversation, or run :nemo question to open a new Nemo conversation as a workspace. Both the editor command line and Nemo's input wrap long questions. In either place, Shift-Enter, Ctrl-Enter, or Alt-Enter inserts a newline; ordinary Enter submits. Pasted exception traces stay in the draft so you can edit before sending.",
                                "!\n:nemo explain this file"),
                        section("Nemo commands",
                                "Inside Nemo, :conversations, :workers, :new, :switch, :rename, :clear, :reset, :delete, and :abort manage Nemo conversations and delegated workers. :clear removes the visible chat history for the current conversation, while :reset can target another conversation by id. :shells lists asynchronous shell commands Nemo has started for this workspace, and :shell_delete shell-1 stops and removes one. :permissions shows or changes tool permissions, :mcp lists MCP servers, :approvals lists approvals, and :swim-help reads this editor help.",
                                ":clear\n:shells\n:shell_delete shell-1"),
                ...
                chapter("configuration", "Configuration, Reloading, and Customization",
                        ...
                        section("Editor config",
                                "~/.swim/config.json stores editor options, startup commands, normal-mode remaps, and color themes. Startup commands run after the window is initialized. Keep remaps small and explicit so the top context bar and command grammar remain understandable.",
                                "~/.swim/config.json"),
                        section("Color themes",
                                "Set theme.colors in ~/.swim/config.json to override editor color roles. Role names are lower-case words separated by dots, such as mode.normal, text.primary, surface.background, command.foreground, diagnostic.error.background, semantic.keyword, completion.kind.function, mail.unread.foreground, and git.commit.hash. A value can be a literal color like #ff00ff or another role name like accent.gold. Core views, language semantic colors, debugger panels, mail/chat/todo surfaces, and plugin panel spans all resolve through these roles. Plugins can also use custom role names; define that role in theme.colors to make it available.",
                                "{\n  \"theme\": {\n    \"name\": \"high-contrast\",\n    \"colors\": {\n      \"mode.normal\": \"#ff00ff\",\n      \"text.primary\": \"#f8fbff\",\n      \"semantic.keyword\": \"accent.gold\",\n      \"git.commit.hash\": \"accent.orange\"\n    }\n  }\n}"),
                        section("Private plugins",
                                "Company-only plugins can be installed as jars in ~/.swim/private-plugins. That directory is not managed by the public SWIM checkout, so :rebuild and updates leave it untouched. Set SWIM_PRIVATE_PLUGIN_PATH, or the JVM property swim.private-plugin-path, to a path-separated list of additional plugin directories. Private plugins use the normal SwimPlugin API and Nemo approval rules. Only install trusted code: SWIM never searches the project you open for plugins.",
                                "mkdir -p ~/.swim/private-plugins\ncp company-swim-plugin.jar ~/.swim/private-plugins/\n:reload"),
                        section("Session restore",
                                "Normal launches open the requested file list or an editable *scratch* buffer. :reload and :rebuild are different: they preserve the current buffers, workspaces, and split layout while the runtime restarts, so development reloads do not destroy editor state.",
                                ":reload\n:rebuild"),
