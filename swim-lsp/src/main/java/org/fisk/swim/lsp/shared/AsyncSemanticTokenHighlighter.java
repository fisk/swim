...
public final class AsyncSemanticTokenHighlighter {
  ...
  public AsyncSemanticTokenHighlighter(
    ...
    _requestDescription =
        requestDescription == null || requestDescription.isBlank()
  ...
  private void enqueueRefresh(Snapshot snapshot, Document document, long delayMillis) {
    _requestQueue.schedule(_requestDescription, () -> refresh(snapshot, document), delayMillis);
  }
  ...
  private void refresh(Snapshot snapshot, Document document) {
    ...
    if (!highlights.isEmpty()) {
      synchronized (this) {
        ...
        _semanticTokensCache.put(
            snapshot.uri(),
            new CachedSemanticTokens(
                currentVersion, clampHighlights(highlights, document.text().length())));
  ...
  public static List<Highlight> decodeSemanticHighlights(
    ...
    if (text == null
        ...
        || tokens.getData() == null
        || legend == null
        || legend.getTokenTypes() == null
        || colorMapper == null) {
      return List.of();
    }
    var tokenTypes = legend.getTokenTypes();
    var tokenModifiers =
        legend.getTokenModifiers() == null ? List.<String>of() : legend.getTokenModifiers();
    ...
    for (int i = 0; i + 4 < data.size(); i += 5) {
      ...
      try {
        TextColor color =
            colorMapper.colorFor(tokenTypes.get(tokenTypeIndex), modifiersBitset, tokenModifiers);
        ...
        if (log != null) {
          log.debug(
              "Skipping invalid semantic token at line " + line + " character " + character, e);
        }
      }
    }
  ...
  public static List<Highlight> transformInsert(
      List<Highlight> highlights, int position, int length) {
    ...
    for (var highlight : highlights) {
      if (highlight.end() <= position) {
        ...
        if (highlight.start() >= position) {
          ...
          transformed.add(
              new Highlight(
                  highlight.start(), highlight.end() + length, highlight.foregroundColor()));
        }
      }
    }
  ...
  public static List<Highlight> merge(List<Highlight> highlights) {
    ...
    for (var highlight : sorted) {
      if (!merged.isEmpty()) {
        ...
        if (last.foregroundColor().equals(highlight.foregroundColor())
            && highlight.start() <= last.end()) {
          merged.set(
              lastIndex,
              new Highlight(
                  last.start(), Math.max(last.end(), highlight.end()), last.foregroundColor()));
          continue;
        }
      }
  ...
  /**
   * Semantic replies can outlive their source buffer during a reload. Keep stale ranges from
   * escaping the current document before they reach the attributed-string formatter.
   */
