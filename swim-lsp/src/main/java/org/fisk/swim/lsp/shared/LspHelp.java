        ...
        return new SwimHelpChapter("lsp", "Language Server Features",
                ...
                List.of(
                        ...
                        section("Jumping through code",
                                "Use g d for the fast definition jump and g r for references. Related LSP navigation commands are g D for a declaration, g y for a type definition, and g i for an implementation. For example, on an interface method, g i can take you to implementations while g d may stay on the interface declaration.",
                                "gd\ngr\ngD\ngy\ngi"),
                        section("Finding symbols and highlights",
                                "Use Space-comma-H when you want every read or write of the current symbol highlighted in the current document. Use g s to list document symbols such as classes, functions, methods, and fields in the current file. Use g S when you remember a symbol name but not the file; SWIM opens a live workspace-symbol search panel with results that update as you type.",
                                "<SPACE> , H\ngs\ngS"),
