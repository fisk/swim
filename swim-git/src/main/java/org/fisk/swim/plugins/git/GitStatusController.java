...
final class GitStatusController {
  ...
  String title() {
    return switch (_mode) {
      ...
      case DIFF ->
          _diffChange != null
              ? "Git Diff: " + _diffChange.relativePath()
              : _diffTitle == null ? "Git Diff" : _diffTitle;
      case COMMIT -> "Git Commit";
      case HUNK_EDIT ->
          _diffChange == null ? "Git Hunk Edit" : "Git Hunk Edit: " + _diffChange.relativePath();
      case RESOLVER ->
          _resolverState == null
              ? "Git Merge Resolver"
      ...
      case STATUS ->
          _snapshot.repositoryRoot() == null
              ? "Git"
              : "Git: " + _snapshot.repositoryRoot().getFileName();
    };
  }
  ...
  List<String> render(int width, int height) {
    return renderRich(width, height).stream().map(SwimPanelLine::text).toList();
  }
  ...
  private List<String> renderStatus(int height) {
    ...
    var rows = statusRows();
    StatusRow selected =
        rows.isEmpty() ? null : rows.get(Math.max(0, Math.min(_selection, rows.size() - 1)));
    lines.add(
        " "
            + _snapshot.branch()
            + "  "
            + (_message == null ? _snapshot.statusMessage() : _message));
      ...
      rows =
          List.of(
              new StatusRow(
                  GitSection.UNSTAGED,
                  new GitFileChange(
                      GitSection.UNSTAGED, "(clean)", _snapshot.repositoryRoot(), " ")));
    }
  ...
  private List<SwimPanelLine> renderStatusRich(int height) {
    ...
    if (!_snapshot.hasRepository()) {
      ...
      lines.add(
          rich(span(" " + GitKeyBindings.helpLine(GitKeyBindings.View.STATUS), C_MUTED, C_PANEL)));
    ...
    for (int i = 0; i < bodyHeight; i++) {
      ...
      lines.add(
          index < rows.size()
              ? formatStatusRowRich(rows.get(index), index == _selection)
              : SwimPanelLine.plain(""));
    }
    return lines;
  }
  ...
  private SwimPanelLine statusBranchLine() {
    ...
    String messageColor =
        _message == null && _snapshot.isClean() ? C_GREEN : _message == null ? C_GOLD : C_RED;
    return rich(
  ...
  private List<String> renderHistory(int height) {
    ...
    for (int i = 0; i < bodyHeight; i++) {
      int index = _scroll + i;
      lines.add(
          index < _historyEntries.size()
              ? (index == _historySelection ? "> " : "  ")
                  + _historyEntries.get(index).displayLabel()
              : "");
    }
    return lines;
  }
  ...
  private List<SwimPanelLine> renderHistoryRich(int height) {
    ...
    lines.add(
        rich(
            span(" " + GitKeyBindings.helpLine(GitKeyBindings.View.HISTORY), C_MUTED, C_ELEVATED)));
    if (_historyEntries.isEmpty()) {
    ...
    for (int i = 0; i < bodyHeight; i++) {
      ...
      if (index < _historyEntries.size()) {
        ...
        lines.add(
            highlightedTokenLine(
                selected ? "> " : "  ",
                entry.displayLabel(),
                entry.shortId(),
                selected ? C_TEXT : C_MUTED,
                C_COMMIT_HASH,
  ...
  private List<String> renderRebase(int height) {
    ...
    if (_rebaseSelection < _scroll) {
      ...
      if (_rebaseSelection >= _scroll + bodyHeight) {
        _scroll = _rebaseSelection - bodyHeight + 1;
      }
    }
    _scroll =
        Math.max(
            0, Math.min(_scroll, Math.max(0, _rebaseEntries.size() - Math.max(1, bodyHeight))));
    for (int i = 0; i < bodyHeight; i++) {
      int index = _scroll + i;
      lines.add(
          index < _rebaseEntries.size()
  ...
  private List<SwimPanelLine> renderRebaseRich(int height) {
    ...
    lines.add(
        highlightedTokenLine(
            " upstream ",
            upstreamLabel,
            _rebaseUpstreamLabel == null ? "" : firstToken(upstreamLabel),
            C_MUTED,
            C_COMMIT_HASH,
            C_PANEL));
    lines.add(
        rich(span(" " + GitKeyBindings.helpLine(GitKeyBindings.View.REBASE), C_MUTED, C_ELEVATED)));
    int bodyHeight = Math.max(0, height - 3);
    if (_rebaseEntries.isEmpty()) {
  ...
  private List<String> renderDiff(int height) {
    ...
    lines.add(title());
    String hunkInfo =
        _diffView != null && _diffView.hasHunks()
  ...
  private List<String> renderResolver(int height) {
    ...
    appendSection(
        lines,
        "BASE",
        block.base().isEmpty() ? List.of("(not available in markers)") : block.base());
  ...
  private List<String> renderPullRequests(int height) {
    ...
    String remote =
        currentPullRepository() == null ? "no GitHub remote" : currentPullRepository().label();
    lines.add(
        " repo: "
            + remote
            + "  name: "
            + (_pullFilterName.isBlank() ? "(any)" : _pullFilterName)
            + "  labels: "
            + (_pullFilterLabels.isBlank() ? "(any)" : _pullFilterLabels)
            + "  author: "
            + (_pullFilterAuthor.isBlank() ? "(any)" : _pullFilterAuthor));
    ...
    int start =
        Math.max(0, Math.min(_scroll, Math.max(0, _pullRequests.size() - visiblePullRequests)));
    _scroll = start;
  ...
  private List<SwimPanelLine> renderPullRequestsRich(int height) {
    ...
    lines.add(
        rich(
            span(
                " " + GitKeyBindings.helpLine(GitKeyBindings.View.PULL_REQUESTS),
                C_MUTED,
                C_ELEVATED)));
    ...
    if (_pullRequests.isEmpty()) {
      String message =
          _allPullRequests.isEmpty() ? " No pull requests " : " No pull requests match the filter ";
    ...
    for (int i = 0; i < visiblePullRequests && lines.size() < height; i++) {
      ...
      if (index < _pullRequests.size()) {
        ...
        if (lines.size() < height) {
          lines.add(
              pullRequestMetaLine(
                  selected ? "  " : "  ", pullRequest, selected ? C_SELECTION_BG : C_PANEL));
        }
      }
    }
    appendBlankRichLines(lines, height);
    ...
    for (int row = 0; row < bodyRows; row++) {
      lines.add(
          joinReviewPanes(
              row < sidebar.size()
                  ? sidebar.get(row)
                  : paddedLine("", sidebarWidth, C_MUTED, C_PANEL),
  ...
  private List<SwimPanelLine> reviewSidebar(int width, int height) {
    ...
    int start =
        Math.max(
            0,
            Math.min(
                _reviewFileSelection - Math.max(0, rows / 2),
    ...
    for (int row = 0; row < rows; row++) {
      ...
      lines.add(
          paddedLine(
              text, width, selected ? C_TEXT : C_MUTED, selected ? C_SELECTION_BG : C_PANEL));
    }
    return lines;
  }
  ...
  private List<SwimPanelLine> reviewBody(int width, int height) {
    ...
    lines.add(
        paddedLine(
            " "
                + GitKeyBindings.helpLine(GitKeyBindings.View.PULL_REVIEW)
                + "  mode "
                + _reviewMode.label(),
            width,
            C_MUTED,
    ...
    lines.add(
        paddedLine(
            " "
                + (_reviewFileSelection + 1)
                + "/"
                + _reviewFiles.size()
                + "  "
                + file.displayLabel()
                + "  comments "
                + commentsFor(file.path()).size(),
            width,
            C_TEXT,
            C_PANEL));
    int contentHeight = Math.max(0, height - lines.size());
    List<SwimPanelLine> content =
        switch (_reviewMode) {
    ...
    for (int row = 0; row < contentHeight; row++) {
      ...
      lines.add(
          index < content.size() ? content.get(index) : paddedLine("", width, C_MUTED, C_PANEL));
    }
    return lines;
  }
          ...
          if (patchLine.startsWith("+") && !patchLine.startsWith("+++")) {
            lines.add(
                rich(
                    ...
                    span(" " + REVIEW_SEPARATOR + " ", C_SUBTLE, C_PANEL),
                    span(
                        padRight(fit(patchLine, rightWidth), rightWidth),
                        C_INSERT_FG,
                        C_INSERT_BG)));
          } else {
  ...
  private void appendReviewComments(
      List<SwimPanelLine> lines, GitHubPullRequestFile file, int width) {
    List<GitHubReviewComment> comments = commentsFor(file.path());
    if (comments.isEmpty()) {
      return;
    }
    ...
    lines.add(
        paddedLine(
            " @"
                + comment.author()
                + "  "
                + comment.locationLabel()
                + (comment.createdAt().isBlank() ? "" : "  " + comment.createdAt()),
            width,
            C_ACCENT,
            C_ELEVATED));
  ...
  private List<GitHubReviewComment> issueComments() {
    return _reviewComments.stream().filter(GitHubReviewComment::issueComment).toList();
  }
  ...
  private static List<SwimPanelLine> plainLines(List<String> lines) {
    return lines.stream().map(SwimPanelLine::plain).toList();
  }

  private static SwimPanelLine joinReviewPanes(
      SwimPanelLine left, SwimPanelLine right, int leftWidth) {
  ...
  private static SwimPanelLine paddedLine(
      String text, int width, String foreground, String background) {
    return rich(span(padRight(fit(text, width), width), foreground, background));
  }

  private static SwimPanelLine paddedRichLine(
      int width, String padForeground, String padBackground, SwimTextSpan... spans) {
    if (width <= 0) {
  ...
  private static SwimPanelLine highlightedTokenLine(
      String prefix,
      String label,
      String token,
      String foreground,
  ...
  private SwimPanelLine pullSavedViewLine() {
    ...
    for (int i = 0; i < _pullSavedViews.size(); i++) {
      ...
      spans.add(
          span(
              selected ? "[" + view.displayLabel() + "]" : view.displayLabel(),
              selected ? C_ACCENT : C_MUTED,
              selected ? C_SELECTION_BG : C_PANEL));
    }
    return new SwimPanelLine(spans);
  }

  private void appendFilterFieldSpans(
      List<SwimTextSpan> spans, PullFilterField field, String value, String valueColor) {
    ...
    spans.add(
        span(
            value.isBlank() ? "(any)" : value, value.isBlank() ? C_MUTED : valueColor, background));
  }
  ...
  private static SwimPanelLine pullRequestTitleLine(
      String prefix, GitHubPullRequest pullRequest, String foreground, String background) {
    var spans = new ArrayList<SwimTextSpan>();
    spans.add(span(prefix, C_MUTED, background));
  ...
  private static SwimPanelLine pullRequestMetaLine(
      String prefix, GitHubPullRequest pullRequest, String background) {
    var spans = new ArrayList<SwimTextSpan>();
    spans.add(span(prefix, C_MUTED, background));
    spans.add(span("@", C_MUTED, background));
  ...
  private SwimPanelResult handleHistoryInput(String input) {
    ...
    return switch (input) {
      ...
      case "enter", "d" -> {
        try {
          yield showText(
              "Git Commit: " + _historyEntries.get(_historySelection).shortId(),
              GitStatusService.commitText(
  ...
  private SwimPanelResult handleDiffInput(String input) {
    return switch (input) {
      ...
      case "enter" ->
          _diffChange != null && java.nio.file.Files.exists(_diffChange.absolutePath())
  ...
  private SwimPanelResult handleHunkEditInput(String input) {
    switch (input) {
      case "ctrl-s":
        try {
          GitStatusService.applyPatch(
              _snapshot.repositoryRoot(), currentEditedPatch(), _editOperation);
          refresh();
  ...
  private SwimPanelResult handlePullRequestsInput(String input) {
    ...
    return switch (input) {
      case "j", "down" -> {
        _pullRequestSelection =
            Math.min(Math.max(0, _pullRequests.size() - 1), _pullRequestSelection + 1);
        yield SwimPanelResult.success();
      }
      case "k", "up" -> {
  ...
  private void moveReviewFile(int delta) {
    ...
    _reviewFileSelection =
        Math.max(0, Math.min(_reviewFileSelection + delta, _reviewFiles.size() - 1));
    _reviewScroll = 0;
  }
  ...
  private List<GitFileChange> changesWithCode(String... statusCodes) {
    ...
    for (GitFileChange change : _snapshot.staged()) {
      if (codes.contains(change.statusCode())) {
        changes.add(change);
      }
    }
    for (GitFileChange change : _snapshot.unstaged()) {
      if (codes.contains(change.statusCode())) {
        changes.add(change);
      }
    }
    for (GitFileChange change : _snapshot.untracked()) {
      if (codes.contains(change.statusCode())) {
        changes.add(change);
      }
    }
    changes.sort(
        java.util.Comparator.comparing(GitFileChange::relativePath)
  ...
  private void appendNonEmptySection(List<StatusRow> rows, GitSection section, List<?> entries) {
    if (!entries.isEmpty()) {
      appendSection(rows, section, entries);
    }
  }
  ...
  private void adjustScroll(int bodyHeight, int rowsSize) {
    if (_selection < _scroll) {
      ...
      if (_selection >= _scroll + bodyHeight) {
  ...
  private void adjustHistoryScroll(int bodyHeight) {
    if (_historySelection < _scroll) {
      ...
      if (_historySelection >= _scroll + bodyHeight) {
        _scroll = _historySelection - bodyHeight + 1;
      }
    }
    _scroll =
        Math.max(
            0, Math.min(_scroll, Math.max(0, _historyEntries.size() - Math.max(1, bodyHeight))));
  }
  ...
  private SwimPanelResult toggleOrAdjustSection(StatusRow row, String input) {
    ...
    if ("left".equals(input)) {
      ...
      if ("right".equals(input)) {
        ...
        if (_collapsed.contains(row.section())) {
          _collapsed.remove(row.section());
        } else {
          _collapsed.add(row.section());
        }
  ...
  private SwimPanelResult openPullRequestBrowser() {
    ...
    try {
      ...
      syncActivePullViewName();
      _pullRepositorySelection =
          Math.max(
              0, Math.min(_pullRepositorySelection, Math.max(0, _pullRepositories.size() - 1)));
  ...
  private void applyPullRequestFilter() {
    _pullRequests =
        GitHubPullRequestService.filterPullRequests(_allPullRequests, currentPullFilters());
    _pullRequestSelection =
        Math.max(0, Math.min(_pullRequestSelection, Math.max(0, _pullRequests.size() - 1)));
  ...
  private SwimPanelResult savePullView() {
    ...
    GitPullRequestSavedView next =
        new GitPullRequestSavedView(name, repository.remoteName(), currentPullFilters());
  ...
  private SwimPanelResult applyAdjacentPullView(int delta) {
    ...
    int next =
        index < 0
            ? (delta >= 0 ? 0 : _pullSavedViews.size() - 1)
    ...
    if (view.pinsRemote() && !hasPullRepositoryRemoteName(view.remoteName())) {
      return SwimPanelResult.successMessage(
          "Saved PR view remote not available: " + view.remoteName());
    }
    boolean switchedRemote =
        view.pinsRemote() && selectPullRepositoryByRemoteName(view.remoteName());
  ...
  private SwimPanelResult deleteActivePullView() {
    ...
    List<GitPullRequestSavedView> views =
        _pullSavedViews.stream().filter(filter -> !filter.name().equals(deleted)).toList();
    try {
      GitProjectSwimConfig.savePullRequestViews(_snapshot.repositoryRoot(), views);
      _pullSavedViews = List.copyOf(views);
      _pullActiveViewName = null;
  ...
  private void syncActivePullViewName() {
    ...
    for (GitPullRequestSavedView view : _pullSavedViews) {
      if (view.filters().equals(filters)
          && (!view.pinsRemote() || view.remoteName().equals(remoteName))) {
        _pullActiveViewName = view.name();
        return;
      }
    }
  ...
  private void selectPullRepository(int delta) {
    ...
    _pullRepositorySelection =
        Math.floorMod(_pullRepositorySelection + delta, _pullRepositories.size());
    _pullActiveViewName = null;
    _allPullRequests = List.of();
    _pullRequests = List.of();
    _pullRequestSelection = 0;
  ...
  private SwimPanelResult fetchSelectedPullRequest() {
    ...
    try {
      return SwimPanelResult.successMessage(
          GitHubPullRequestService.fetch(
                  _snapshot.repositoryRoot(), currentPullRepository(), pullRequest)
  ...
  private SwimPanelResult fetchReviewPullRequest() {
    ...
    try {
      return SwimPanelResult.successMessage(
          GitHubPullRequestService.fetch(
              _snapshot.repositoryRoot(), _reviewRepository, _reviewPullRequest));
    } catch (IOException | GitAPIException e) {
      return SwimPanelResult.successMessage(e.getMessage());
    }
  }
  ...
  private SwimPanelResult openPullReview() {
    ...
    try {
      ...
      return commentsMessage == null
          ? SwimPanelResult.success()
          : SwimPanelResult.successMessage(commentsMessage);
    } catch (IOException e) {
      return SwimPanelResult.successMessage(e.getMessage());
    } catch (InterruptedException e) {
      Thread.currentThread().interrupt();
  ...
  private SwimPanelResult openReviewFile() {
    ...
    Path path =
        _snapshot
            .repositoryRoot()
            .resolve(_reviewFiles.get(_reviewFileSelection).path())
            .normalize();
    return java.nio.file.Files.exists(path)
        ? SwimPanelResult.success(path)
  ...
  private GitHubRepository currentPullRepository() {
    ...
    return _pullRepositories.get(
        Math.max(0, Math.min(_pullRepositorySelection, _pullRepositories.size() - 1)));
  }
  ...
  private GitHubPullRequest currentPullRequest() {
    ...
    return _pullRequests.get(
        Math.max(0, Math.min(_pullRequestSelection, _pullRequests.size() - 1)));
  }
  ...
  private SwimPanelResult openInteractiveRebase() {
    ...
    try {
      ...
      _rebaseEntries =
          GitStatusService.interactiveRebaseEntries(
              _snapshot.repositoryRoot(), upstream.objectId());
      _rebaseSelection = 0;
  ...
  private SwimPanelResult applyInteractiveRebase() {
    ...
    try {
      GitStatusService.runInteractiveRebase(
          _snapshot.repositoryRoot(), _rebaseUpstreamId, _rebaseEntries);
      refresh();
      _mode = Mode.STATUS;
  ...
  private SwimPanelResult resolveConflict(StatusRow row, String resolution) {
    ...
    try {
      switch (resolution) {
        ...
        case "theirs" ->
            GitStatusService.resolveConflictWithTheirs(_snapshot.repositoryRoot(), change);
  ...
  private SwimPanelResult openResolver(StatusRow row) {
    ...
    try {
      ...
      return SwimPanelResult.openMergeResolution(
          new SwimMergeResolution(
              change.absolutePath(),
              _resolverState.oursDocument(),
              _resolverState.theirsDocument(),
              _resolverState.oursRanges(),
  ...
  private String renderDiffLine(int lineIndex) {
    ...
    return lineIndex >= hunk.displayStartLine() && lineIndex <= hunk.displayEndLine()
        ? "> " + line
        : "  " + line;
  }
  ...
  private SwimPanelResult beginHunkEdit() {
    ...
    _editLines =
        new ArrayList<>(
            List.of(_diffView.hunks().get(_currentHunkIndex).patchText().split("\\R", -1)));
    _editRow = 0;
    _editColumn = 0;
  ...
  private SwimPanelResult applyResolverResult() {
    ...
    try {
      ...
      GitStatusService.resolveConflictWithBoth(
          _snapshot.repositoryRoot(),
