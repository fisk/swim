...
public final class MarkdownLanguageMode implements LanguageMode {
  ...
  private static final Pattern EMPHASIS =
      Pattern.compile("(?<!\\w)(?:\\*{1,3}|_{1,3}).+?(?:\\*{1,3}|_{1,3})(?!\\w)");
  ...
  @Override
  public void didInsert(BufferContext context, int position, String text) {
    update(context);
  }

  @Override
  public void didRemove(BufferContext context, int start, int end) {
    update(context);
  }

  @Override
  public void willSave(BufferContext context) {}

  @Override
  public void didSave(BufferContext context) {}

  @Override
  public void didClose(BufferContext context) {}

  @Override
  public void didOpen(BufferContext context) {
    update(context);
  }

  @Override
  public int getIndentationLevel(BufferContext context) {
    return 0;
  }

  @Override
  public boolean isIndentationEnd(BufferContext context, String character) {
    return false;
  }

  @Override
  public TextDocumentItem getTextDocument(BufferContext context) {
    return null;
  }

  @Override
  public boolean trimTrailingWhitespaceOnSave(BufferContext context) {
    return true;
  }
  ...
  private static void colourFences(AttributedString text, String source) {
    ...
    while (matcher.find()) {
      ...
      if (!language.isBlank()) {
        colourEmbedded(text, bodyStart, bodyEnd, language);
      }
      if (close < 0) {
        break;
      }
  ...
  private static void colourEmbedded(AttributedString target, int start, int end, String language) {
    ...
    if (!LanguagePluginRegistry.applySnippetColouring(language, embedded)) {
      return;
    }
    int offset = start;
  ...
  private static void colour(AttributedString text, Pattern pattern, TextColor colour) {
    ...
    while (matcher.find()) {
      text.format(matcher.start(), matcher.end(), colour, TextColor.ANSI.DEFAULT);
    }
  }
  ...
  private void update(BufferContext context) {
    ...
    var snapshot =
        new MarkdownSyntaxAnalysisService.Snapshot(
            buffer.getPath(),
            buffer.getVersion(),
            buffer.getString(),
            TreeSitterBundledGrammars.load("markdown"),
            ...
            EventThread.getInstance()
                .enqueue(
                    new org.fisk.swim.event.RunnableEvent(
                        () -> {
                          if (buffer.getVersion() != result.snapshot().version()) {
                            return;
                          }
                          var ranges =
                              result.spans().stream()
                                  .map(
                                      span ->
                                          new AttributedString.FormatRange(
                                              span.start(),
                                              span.end(),
                                              "markdown.block".equals(span.type())
                                                      || "markdown.fence".equals(span.type())
                                                  ? UiTheme.SEMANTIC_KEYWORD
                                                  : UiTheme.SEMANTIC_STRING,
                                              null))
                                  .toList();
