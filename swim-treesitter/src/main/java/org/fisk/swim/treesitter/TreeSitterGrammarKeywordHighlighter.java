 ...
 * separate from the constrained structural runtime: recursive production grammars remain usable for
 * immediate keyword colouring while their full LR parser support is under development.
 */
public final class TreeSitterGrammarKeywordHighlighter {
  ...
  public TreeSitterGrammarKeywordHighlighter(TreeSitterGrammar grammar) {
    ...
    var visited =
        java.util.Collections.newSetFromMap(
            new java.util.IdentityHashMap<TreeSitterGrammarRule, Boolean>());
  ...
  private static void collect(
      TreeSitterGrammarRule rule, Set<String> keywords, Set<TreeSitterGrammarRule> visited) {
    if (!visited.add(rule)) {
      return;
    }
    ...
    rule.children()
        .values()
        .forEach(children -> children.forEach(child -> collect(child, keywords, visited)));
  }
  ...
  private static int quotedEnd(String source, int index, char quote) {
    ...
    while (index < source.length()) {
      if (source.charAt(index++) == '\\' && index < source.length()) {
        index++;
      } else {
