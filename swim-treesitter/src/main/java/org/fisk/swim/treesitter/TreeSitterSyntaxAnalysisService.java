...
public final class TreeSitterSyntaxAnalysisService implements AutoCloseable {
  public record Result(
      TreeSitterSyntaxSnapshot snapshot, TreeSitterSyntaxTree tree, Throwable failure) {
    public boolean succeeded() {
      return failure == null && tree != null && tree.succeeded();
    }
  }
  ...
  private final ConcurrentHashMap<String, Long> _latestRequestByDocument =
   ...
   */
  public long parse(
      String documentKey, TreeSitterSyntaxSnapshot snapshot, Consumer<Result> consumer) {
    ...
    if (_closed.get() || _workers.isClosed()) {
      throw new IllegalStateException("Tree-sitter syntax analysis is closed");
    }
    ...
    _workers.start(
        () -> {
          ...
          if (!_closed.get()
              && Long.valueOf(request).equals(_latestRequestByDocument.get(documentKey))) {
  ...
  private static String documentKey(TreeSitterSyntaxSnapshot snapshot) {
    return snapshot.path() == null
        ? "untitled:" + System.identityHashCode(snapshot)
