...
public final class TreeSitterGrammarKeywordBridge implements AutoCloseable {
  ...
  public TreeSitterGrammarKeywordBridge(
      SwimPluginWorkers workers, Consumer<Runnable> uiDispatcher, TreeSitterGrammar grammar) {
    _workers = workers == null ? SwimPluginWorkers.unmanaged() : workers;
    _uiDispatcher = Objects.requireNonNull(uiDispatcher, "uiDispatcher");
    _highlighter =
        new TreeSitterGrammarKeywordHighlighter(Objects.requireNonNull(grammar, "grammar"));
  }
  ...
  private void update(Buffer buffer) {
    ...
    var snapshot =
        new TreeSitterGrammarSnapshot(buffer.getPath(), buffer.getVersion(), buffer.getString());
    ...
    _workers.start(
        () -> {
          ...
          if (_closed.get() || !Long.valueOf(request).equals(_latest.get(documentKey))) {
            return;
          }
          var ranges =
              spans.stream()
                  .map(
                      span ->
                          new AttributedString.FormatRange(
                              span.start(), span.end(), UiTheme.SEMANTIC_KEYWORD, null))
                  .toList();
          _uiDispatcher.accept(
              () -> buffer.setSyntaxFormatOverlays((int) snapshot.version(), ranges));
        });
  }
