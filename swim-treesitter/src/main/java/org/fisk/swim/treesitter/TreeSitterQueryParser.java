...
/**
 * Parser for structural Tree-sitter query patterns. Predicates are preserved as nodes but
 * deliberately not evaluated until a query runtime exists.
 */
public final class TreeSitterQueryParser {
          ...
          if (isPredicate()) {
            ...
            if (peek("(")) {
              ...
              if (peek("[")) {
                ...
                throw error("Invalid Tree-sitter query wrapper content");
              }
            }
          }
        }
  ...
  private TreeSitterQueryNode alternatives() {
    ...
    while (!peek("]")) {
      if (atEnd()) {
        throw error("Unterminated Tree-sitter query alternation");
      }
  ...
  private boolean isPredicate() {
    return peek("(")
        && _index + 1 < _tokens.size()
        && _tokens.get(_index + 1).value().startsWith("#");
  }
  ...
  private TreeSitterQueryPredicate predicate() {
    ...
    while (!peek(")")) {
      if (atEnd()) {
        throw error("Unterminated Tree-sitter query predicate");
      }
      ...
      if (Character.isWhitespace(current)) {
        ...
        if (current == ';') {
          ...
          if (current == '('
            ...
            if (current == '@') {
              int start = index++;
              while (index < input.length() && isWord(input.charAt(index))) {
                index++;
              }
