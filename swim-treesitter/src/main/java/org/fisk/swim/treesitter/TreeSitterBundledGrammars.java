...
public final class TreeSitterBundledGrammars {
    ...
    try (InputStream input = TreeSitterBundledGrammars.class.getResourceAsStream(resource)) {
      if (input == null) {
        throw new IllegalArgumentException(
            "No bundled Tree-sitter query " + query + " for " + language);
      }
      return new String(input.readAllBytes(), StandardCharsets.UTF_8);
    } catch (IOException e) {
      throw new IllegalStateException(
          "Failed to read bundled Tree-sitter query for " + language, e);
    }
  }
  ...
  private static TreeSitterGrammar read(String language) {
    ...
    try (InputStream input = TreeSitterBundledGrammars.class.getResourceAsStream(resource)) {
      if (input == null) {
        throw new IllegalArgumentException("No bundled Tree-sitter grammar for " + language);
      }
      String text = new String(input.readAllBytes(), StandardCharsets.UTF_8);
      TreeSitterGrammar grammar =
          TreeSitterGrammarInspector.parse(
      ...
      if ("cpp".equals(language)) {
        grammar = inherit(load("c"), grammar);
      }
      return applyExtension(grammar, language);
    } catch (IOException e) {
      throw new IllegalStateException(
          "Failed to read bundled Tree-sitter grammar for " + language, e);
    }
  }
  ...
  private static TreeSitterGrammar inherit(TreeSitterGrammar parent, TreeSitterGrammar child) {
    ...
    return new TreeSitterGrammar(
        child.name(), rules, child.externals(), extras, child.conflicts(), child.wordToken());
  }
  ...
  private static TreeSitterGrammar applyExtension(TreeSitterGrammar grammar, String language) {
    ...
    rules.put(
        "__swim_extension_keywords",
        new TreeSitterGrammarRule(
            "CHOICE",
            java.util.Map.of(),
            java.util.Map.of(
                "members",
                keywords.stream()
                    .map(
                        keyword ->
                            new TreeSitterGrammarRule(
                                "STRING", java.util.Map.of("value", keyword), java.util.Map.of()))
                    .toList())));
    return new TreeSitterGrammar(
        grammar.name(),
        rules,
