...
/**
 * Markdown's upstream grammar is split into block and inline parsers. This is the immutable
 * asynchronous boundary for that model; the current Java implementation recognizes the portable
 * highlight constructs while the full LR/query runtime is filled in behind the same snapshot API.
 */
public final class MarkdownSyntaxAnalysisService implements AutoCloseable {
  public record Snapshot(
      ...
      long version,
      String text,
      TreeSitterGrammar blockGrammar,
  ...
  public record Result(
      Snapshot snapshot, List<TreeSitterSyntaxSpan> spans, List<TreeSitterQueryCapture> captures) {}
  ...
  public long analyse(String document, Snapshot snapshot, Consumer<Result> consumer) {
    ...
    workers.start(
        () -> {
          ...
          var tree =
              new TreeSitterSyntaxTree(
                  new TreeSitterSyntaxSnapshot(
                      snapshot.path(),
                      snapshot.version(),
                      snapshot.text(),
                      snapshot.blockGrammar(),
                      "document"),
                  spans,
                  List.of());
          ...
          spans.stream()
              .filter(span -> "inline".equals(span.type()))
              .forEach(span -> captures.add(new TreeSitterQueryCapture("text.title", span)));
          spans.stream()
              .filter(span -> span.type().startsWith("atx_h"))
              .forEach(
                  span -> captures.add(new TreeSitterQueryCapture("punctuation.special", span)));
          ...
          spans.stream()
              .filter(span -> "language".equals(span.type()))
              .forEach(
                  span -> captures.add(new TreeSitterQueryCapture("injection.language", span)));
          spans.stream()
              .filter(span -> "code_fence_content".equals(span.type()))
              .forEach(span -> captures.add(new TreeSitterQueryCapture("injection.content", span)));
          if (!closed.get() && Long.valueOf(request).equals(latest.get(document))) {
            consumer.accept(new Result(snapshot, spans, List.copyOf(captures)));
          }
        });
    return request;
  }
  ...
  private static List<TreeSitterSyntaxSpan> scan(String source) {
    ...
    while (headings.find()) {
      ...
      result.add(
          new TreeSitterSyntaxSpan(
              "atx_h" + headings.group(1).length() + "_marker",
              "",
              headings.start(1),
              headings.end(1)));
    ...
    while (fences.find()) {
      ...
      result.add(
          new TreeSitterSyntaxSpan(
              "fenced_code_block_delimiter", "", fences.start(), fences.end()));
      ...
      result.add(
          new TreeSitterSyntaxSpan(
              "code_fence_content", "", fences.end(), close < 0 ? source.length() : close + 1));
    }
  ...
  private static void add(List<TreeSitterSyntaxSpan> spans, Matcher matcher, String type) {
    while (matcher.find()) {
      spans.add(new TreeSitterSyntaxSpan(type, "", matcher.start(), matcher.end()));
    }
  }
