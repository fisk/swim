    ...
    try (InputStream input = TreeSitterGrammarExtensions.class.getResourceAsStream(resource)) {
      ...
      var root =
          JsonParser.parseString(new String(input.readAllBytes(), StandardCharsets.UTF_8))
              .getAsJsonObject();
      if (!language.equals(root.get("language").getAsString())) {
        throw new IllegalArgumentException("Extension language mismatch");
      }
      var values = new java.util.ArrayList<String>();
      if (root.has("missingLiteralKeywords")) {
        for (var value : root.getAsJsonArray("missingLiteralKeywords")) {
