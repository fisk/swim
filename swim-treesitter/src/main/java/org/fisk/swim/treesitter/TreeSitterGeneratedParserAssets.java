...
public final class TreeSitterGeneratedParserAssets {
  private static final ConcurrentHashMap<String, TreeSitterGeneratedParserMetadata> METADATA =
  ...
  private static TreeSitterGeneratedParserMetadata readMetadata(String language) {
    ...
    try (InputStream input = TreeSitterGeneratedParserAssets.class.getResourceAsStream(resource)) {
      if (input == null) {
        throw new IllegalArgumentException("No bundled generated parser for " + language);
      }
      return TreeSitterGeneratedParserInspector.inspect(
