...
/**
 * Experimental Java executor for a small, explicit subset of generated Tree-sitter grammars. It is
 * not an LR/GLR Tree-sitter replacement: recursive symbols and unsupported generated rule forms
 * fail explicitly.
 */
public final class TreeSitterSubsetRuntime {
  public TreeSitterSyntaxTree parse(TreeSitterSyntaxSnapshot snapshot) {
    ...
    if (!compatibility.supported()) {
      return new TreeSitterSyntaxTree(
          snapshot,
          List.of(),
          compatibility.issues().stream()
              .map(
                  issue ->
                      new TreeSitterParseError(0, issue.message() + " (" + issue.feature() + ")"))
              .toList());
    }
    if (snapshot.startRule().isBlank()) {
      return new TreeSitterSyntaxTree(
          snapshot, List.of(), List.of(new TreeSitterParseError(0, "Grammar has no start rule")));
    }
    ...
    try {
      ...
      return new TreeSitterSyntaxTree(
          snapshot,
          List.of(),
          List.of(new TreeSitterParseError(failure.offset, failure.getMessage())));
  ...
  private static List<TreeSitterSyntaxSpan> sort(List<TreeSitterSyntaxSpan> spans) {
    return spans.stream()
        .sorted(
            Comparator.comparingInt(TreeSitterSyntaxSpan::start)
                .thenComparing((TreeSitterSyntaxSpan span) -> -span.end()))
        .toList();
  }
  ...
  private static final class Parser {
    ...
    private Parser(TreeSitterSyntaxSnapshot snapshot) {
      this.snapshot = snapshot;
    }

    private Match namedRule(String name, int offset, String field) {
      if (symbols.contains(name)) {
        throw unsupported(offset, "recursive symbol '" + name + "'");
      }
      TreeSitterGrammarRule rule = snapshot.grammar().rules().get(name);
      if (rule == null) {
        throw new ParseFailure(offset, "Unknown grammar symbol '" + name + "'");
      }
      symbols.push(name);
      try {
    ...
    private Match rule(TreeSitterGrammarRule rule, int offset, String field) {
      ...
      return switch (rule.type()) {
        ...
        case "ALIAS" ->
            alias(single(rule, "content", start), start, field, rule.attribute("value"));
    ...
    private TreeSitterGrammarRule single(TreeSitterGrammarRule rule, String child, int offset) {
      ...
      if (values.size() != 1) {
        throw unsupported(offset, "'" + rule.type() + "' requires one " + child + " rule");
      }
      return values.getFirst();
    }

    private Match literal(String value, int offset, String field) {
      if (value.isEmpty() || !snapshot.text().startsWith(value, offset)) {
        throw new ParseFailure(offset, "Expected '" + value + "'");
      }
      return new Match(
          offset + value.length(),
          List.of(new TreeSitterSyntaxSpan("STRING", field, offset, offset + value.length())));
    }

    private Match pattern(String source, int offset, String field) {
      if (source.isEmpty()) {
        throw unsupported(offset, "empty PATTERN rule");
      }
      var matcher = Pattern.compile(source).matcher(snapshot.text());
      matcher.region(offset, snapshot.text().length());
      if (!matcher.lookingAt()) {
        throw new ParseFailure(offset, "Expected pattern '" + source + "'");
      }
      if (matcher.end() == offset) {
        throw unsupported(offset, "zero-width PATTERN rule");
      }
      return new Match(
          matcher.end(),
          List.of(new TreeSitterSyntaxSpan("PATTERN", field, offset, matcher.end())));
    }
    ...
    private Match choice(List<TreeSitterGrammarRule> members, int offset, String field) {
      ...
      for (TreeSitterGrammarRule member : members) {
        try {
          ...
          if (candidateFailure.offset > failure.offset) {
    ...
    private Match repeat(
        TreeSitterGrammarRule content, int offset, String field, boolean atLeastOne) {
      int end = offset;
      int count = 0;
      var spans = new ArrayList<TreeSitterSyntaxSpan>();
      while (true) {
        try {
          ...
          if (match.end() <= end) {
            throw unsupported(end, "zero-width repeated rule");
          }
          end = match.end();
          count++;
          spans.addAll(match.spans());
      ...
      if (atLeastOne && count == 0) {
        throw new ParseFailure(offset, "Expected one or more repeated values");
      }
      return new Match(end, spans);
    }
    ...
    private Match optional(TreeSitterGrammarRule content, int offset, String field) {
      try {
        return rule(content, offset, field);
      } catch (ParseFailure failure) {
        return new Match(offset, List.of());
      }
    }
  ...
  private static final class ParseFailure extends RuntimeException {
    ...
    private ParseFailure(int offset, String message) {
      super(message);
      this.offset = offset;
    }
  }
  ...
  private static final class UnsupportedFeature extends RuntimeException {
    ...
    private UnsupportedFeature(int offset, String message) {
      super(message);
      this.offset = offset;
    }
  }
