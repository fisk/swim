        ...
        return new SwimHelpChapter("cpp-debug", "C/C++ Debugger",
                ...
                List.of(
                        section("Launching C/C++ debugging",
                                "Use :debug cpp with an executable, optional source root, and program arguments. Add --cwd <directory> after the source root to set the debugged program's working directory without changing SWIM's project directory. The plugin chooses gdb or lldb when available, or you can request one explicitly with :debug cpp gdb or :debug cpp lldb. Build the executable first, then launch the debugger from a source file in the same project so source paths resolve naturally.",
